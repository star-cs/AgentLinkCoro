...
Tracker::Tracker() : m_datas(nullptr), m_interval(60)
{
}
...
void Tracker::start()
{
    ...
    init();
    auto iom = base::WorkerMgr::GetInstance()->getAsIOManager("timer");
    if (iom) {
        m_timer = iom->addTimer(100, std::bind(&Tracker::onTimer, shared_from_this()), true);
    } else {
        _LOG_INFO(g_logger) << "Woker timer not exists";
        base::IOManager::GetThis()->addTimer(100, std::bind(&Tracker::onTimer, shared_from_this()),
                                             true);
    }
}
...
int64_t Tracker::inc(uint32_t key, int64_t v)
{
    ...
    if (idx < m_dimCount) {
        return base::Atomic::addFetch(m_datas[idx], v);
    }
    return 0;
}
...
int64_t Tracker::dec(uint32_t key, int64_t v)
{
    ...
    auto idx = key * m_interval + now % m_interval;
    if (idx < m_dimCount) {
        return base::Atomic::subFetch(m_datas[idx], v);
    }
    return 0;
}
...
int64_t Tracker::privateInc(uint32_t key, int64_t v)
{
    ...
    if (key < m_dimCount) {
        return base::Atomic::subFetch(m_datas[key], v);
    }
    return 0;
}
...
void Tracker::toDurationData(std::map<std::string, int64_t> &data, uint32_t duration,
                             bool with_time)
{
    time_t now = time(0) - 1;
...
{
    if (m_timer) {
        _LOG_ERROR(g_logger) << "Tracker is running, addDim key=" << key << " name=" << name;
        return;
    }
    if (key >= m_dims.size()) {
        m_dims.resize(key + 1);
...
TrackerManager::TrackerManager() : m_interval(60)
{
}
