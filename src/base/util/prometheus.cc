...
PrometheusRegistry::addCounter(const std::string &name, const std::string &help,
...
PrometheusRegistry::addGauge(const std::string &name, const std::string &help,
                             const std::map<std::string, std::string> &labels)
{
    base::RWMutex::ReadLock lock(m_mutex);
...
PrometheusRegistry::addHistogram(const std::string &name, const std::string &help,
                                 const std::map<std::string, std::string> &labels)
{
    base::RWMutex::ReadLock lock(m_mutex);
...
PrometheusRegistry::addSummary(const std::string &name, const std::string &help,
                               const std::map<std::string, std::string> &labels)
{
    base::RWMutex::ReadLock lock(m_mutex);
...
prometheus::Counter *
PrometheusRegistry::addCounterLabels(const std::string &name,
                                     const std::map<std::string, std::string> &labels)
{
    base::RWMutex::WriteLock lock(m_mutex);
...
prometheus::Gauge *
PrometheusRegistry::addGaugeLabels(const std::string &name,
                                   const std::map<std::string, std::string> &labels)
{
    base::RWMutex::ReadLock lock(m_mutex);
...
PrometheusRegistry::addHistogramLabels(const std::string &name,
                                       const std::map<std::string, std::string> &labels,
...
prometheus::Summary *
PrometheusRegistry::addSummaryLabels(const std::string &name,
                                     const std::map<std::string, std::string> &labels,
...
void PrometheusClient::push()
{
    ...
    for (auto &i : infos) {
        ...
        auto result = base::http::HttpConnection::DoPost(uri, 1000, {}, str);
        _LOG_INFO(g_logger) << result->toString();
    }
}
...
void PrometheusClient::start()
{
    ...
    base::IOManager::GetThis()->schedule([self, this]() {
        while (true) {
            push();
