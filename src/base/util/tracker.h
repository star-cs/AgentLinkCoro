...
public:
    ...
    typedef std::function<void(const std::vector<std::string> &dims, int64_t *datas,
                               int64_t interval)>
        callback;
    ...
    void toDurationData(std::map<std::string, int64_t> &data, uint32_t duration = 1,
                        bool with_time = true);
...
typedef base::Singleton<TrackerManager> TrackerMgr;
