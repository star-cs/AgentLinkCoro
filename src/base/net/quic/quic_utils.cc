...
namespace quic
{
    ...
    void RTTStats::updateRTT(uint64_t send_delta, uint64_t ack_delay, uint64_t now)
    {
        ...
        m_latest_rtt = sample;
        _LOG_DEBUG(g_logger) << "receivedAck updateRTT real_rtt: " << send_delta
                             << ", ack_delay: " << ack_delay << ", min_rtt: " << m_min_rtt
                             << ", sample: " << sample;
        if (!m_has_measurement) {
            ...
            m_mean_deviation =
                ONE_MINUS_BETA * m_mean_deviation + RTT_BETA * abs_diff(m_smoothed_rtt, sample);
            m_smoothed_rtt = ONE_MINUS_ALPHA * m_smoothed_rtt + RTT_ALPHA * sample;
        }
        _LOG_WARN(g_logger) << "trace now: " << GetCurrentUS() << " real_rtt: " << send_delta
                            << " ack_delay: " << ack_delay << " min_rtt: " << m_min_rtt
                            << " sample: " << sample << " m_smoothed_rtt: " << m_smoothed_rtt
    ...
    uint64_t Pacer::maxBurstSize()
    {
        ...
        return std::max(
            ...
            ideal_size, MaxBurstSizePackets * m_max_datagram_size);
    }
    ...
    uint64_t Pacer::budget(uint64_t now)
    {
        ...
        uint64_t budget =
            m_budget_at_last_sent + (m_get_adjust_bw_cb() * (now - m_last_sent_time) / 1000 / 1000);
        return budget;
    ...
    uint64_t Pacer::timeUntilSend()
    {
        ...
        uint64_t ideal_delay = (uint64_t)ceil((m_max_datagram_size - m_budget_at_last_sent) * 1000
                                              * 1000 / (m_get_adjust_bw_cb()));
        _LOG_WARN(g_logger) << "m_last_sent_time: " << m_last_sent_time
                            << ", ideal_delay: " << ideal_delay
                            << " budget_at_last_sent: " << m_budget_at_last_sent;
        return m_last_sent_time + ideal_delay;
        return m_last_sent_time + std::max(min_pacing_delay, ideal_delay);
    }
} // namespace quic
} // namespace base
