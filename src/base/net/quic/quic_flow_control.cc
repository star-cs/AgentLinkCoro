...
namespace quic
{
    ...
    bool BaseFlowController::hasWinUpdate()
    {
        ...
        if (bytes_remaining <= (uint64_t)(float(m_receive_win_size * (1 - 0.25)))) {
            _LOG_DEBUG(g_logger) << "consume more 1/4 * " << m_receive_win_size;
            return true;
        }
        return false;
    }
    ...
    void BaseFlowController::maybeAdjustWinSize()
    {
        ...
        uint64_t now = base::GetCurrentUS();
        uint64_t diff = now - m_epoch_start_time;
        uint64_t base_diff = 4 * fraction * (float)rtt;

        if (diff < base_diff) {
            _LOG_WARN(g_logger) << "aggresive set recv_win_size";
            m_receive_win_size = std::min(2 * m_receive_win_size, m_max_recv_win_size);
        }
        if (debug_tag == "stream") {
            _LOG_WARN(g_logger) << "real_diff: " << diff << ", base_diff: " << base_diff
    ...
    {
        if (m_bytes_read == 0) {
            startNewAutoTuningEpoch(base::GetCurrentUS());
        }
        m_bytes_read += n;
    }
    ...
    uint64_t BaseFlowController::getWinUpdate()
    {
        ...
        if (debug_tag == "stream") {
            _LOG_WARN(g_logger) << "trace now: " << GetCurrentUS() << " getWinUpdate receive_win "
                                << m_receive_win << " receive_win_size " << m_receive_win_size
    ...
    {
        if (sendWinSize() != 0 || m_send_win == m_last_blocked_at) {
            return 0;
        }
    ...
    {
        MutexType::Lock lock(m_mutex);
        if (m_bytes_read > 0 || m_highest_received > 0 || !m_epoch_start_time) {
            return;
        }
        m_bytes_sent = 0;
        m_last_blocked_at = 0;
    ...
    void StreamFlowController::addBytesSent(uint64_t n)
    {
        ...
        if (m_send_win > old_m_send_win) {
            _LOG_WARN(g_logger) << "after stream::addBytesSent: " << m_bytes_sent
                                << " m_send_win: " << m_send_win
                                << ", conn m_bytes_sent: " << m_connection->m_bytes_sent
                                << " m_send_win: " << m_connection->m_send_win;
        }
    }
    ...
    uint64_t StreamFlowController::getWinUpdate()
    {
        ...
        if (m_receive_win_size > old_win_size) {
            _LOG_DEBUG(g_logger) << "Increasing receive flow control window to %d kB";
            m_connection->ensureMinWinSize((uint64_t)(m_receive_win_size * 1.5) // TODO
            );
        }
        lock.unlock();
        return offset;
