...
namespace quic
{
    ...
    QuicStatelessResetToken::QuicStatelessResetToken(const QuicConnectionId &conn_id,
    ...
    {
        return (static_cast<uint64_t>(this->m_token[0]) << 56)
               + (static_cast<uint64_t>(this->m_token[1]) << 48)
               + (static_cast<uint64_t>(this->m_token[2]) << 40)
               + (static_cast<uint64_t>(this->m_token[3]) << 32)
               + (static_cast<uint64_t>(this->m_token[4]) << 24)
               + (static_cast<uint64_t>(this->m_token[5]) << 16)
               + (static_cast<uint64_t>(this->m_token[6]) << 8)
               + (static_cast<uint64_t>(this->m_token[7]));
    }
    ...
    {
        QuicConnectionId::ptr cid = nullptr; // 初始化一个指向 QuicConnectionId 对象的智能指针
        if (buffer_block->readAvailable() == 0) { // 如果缓冲区中没有可读数据
            return nullptr;                       // 返回空指针
        }
        // 判断是否是长头部。QUIC 协议规定，长头部的第一个字节的最高位（0x80）为 1。
        bool is_long_header = (buffer_block->toString()[0] & 0x80) > 0;
        if (!is_long_header) { // 如果是短头部
            // 短头部的连接 ID 长度通常是固定的，这里假设是 4 字节，并从缓冲区的第二个字节开始读取
            cid = std::make_shared<QuicConnectionId>(
                (const uint8_t *)&buffer_block->toString().c_str()[1], 4);
            return cid; // 返回解析出的连接 ID
        }
        // 如果是长头部，并且缓冲区中可读数据小于 6 字节（1字节类型 + 4字节版本 +
        // 1字节目的连接ID长度）
        if (buffer_block->readAvailable() < 6) {
            return nullptr; // 返回空指针
        }
        uint8_t dst_cid_len; // 目的连接 ID 长度
        // 从缓冲区的第 6 个字节（索引为 5）开始读取目的连接 ID 的长度
        memcpy(&dst_cid_len, &buffer_block->toString()[5], 1);
        // 如果缓冲区中可读数据小于 6 字节加上目的连接 ID 的长度
        if ((int)buffer_block->readAvailable() < 6 + dst_cid_len) {
            return nullptr; // 返回空指针
        }
        // 从缓冲区的第 7 个字节（索引为 6）开始读取目的连接 ID 的数据，长度为 dst_cid_len
        cid = std::make_shared<QuicConnectionId>(
            (const uint8_t *)&buffer_block->toString().c_str()[6], dst_cid_len);
        return cid; // 返回解析出的连接 ID
    }
    ...
    QuicConnectionId::QuicConnectionId(const uint8_t *buf, uint8_t len) : m_len(len)
    {
        _ASSERT(len <= QuicConnectionId::MAX_LENGTH);
        memcpy(this->m_id, buf, std::min(static_cast<int>(len), QuicConnectionId::MAX_LENGTH));
    }
    ...
    {
        return (static_cast<uint64_t>(this->m_id[0]) << 56)
               + (static_cast<uint64_t>(this->m_id[1]) << 48)
               + (static_cast<uint64_t>(this->m_id[2]) << 40)
               + (static_cast<uint64_t>(this->m_id[3]) << 32)
               + (static_cast<uint64_t>(this->m_id[4]) << 24)
               + (static_cast<uint64_t>(this->m_id[5]) << 16)
               + (static_cast<uint64_t>(this->m_id[6]) << 8)
               + (static_cast<uint64_t>(this->m_id[7]));
    }
    ...
    std::string QuicConnectionId::toHexString() const
    {
        ...
        for (auto i = 0; i < m_len; i++) {
            ...
            ss << std::setw(2) << std::setfill('0') << std::hex << (int)(uint8_t)m_id[i] << " ";
        }
        return ss.str();
    }
    ...
    QuicFiveTuple::QuicFiveTuple(IPAddress::ptr src, IPAddress::ptr dst, int protocol)
        : m_source(src), m_destination(dst), m_protocol(protocol)
    {
