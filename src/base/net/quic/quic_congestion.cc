...
namespace quic
{
    ...
    bool SlowStart::shouldExitSlowStart(uint64_t latest_rtt, uint64_t min_rtt, uint64_t cong_win)
    {
        if (!m_started) {
            startReceiveRound(m_last_sent_pn);
        }
        if (m_start_found) {
            _LOG_WARN(g_logger) << "trace now: " << GetCurrentUS() << " shouldExitSlowStart true";
            return true;
        }
        m_rtt_sample_count++;
        if (m_rtt_sample_count <= START_MIN_SAMPLES) {
            if (m_cur_min_rtt == 0 || m_cur_min_rtt > latest_rtt) {
        ...
        if (m_rtt_sample_count == START_MIN_SAMPLES) {
            ...
            _LOG_WARN(g_logger) << "trace now: " << GetCurrentUS()
                                << " shouldExitSlowStart sample is 8, m_cur_min_rtt: "
                                << m_cur_min_rtt << " min_rtt: " << min_rtt
                                << " min_rtt_inc_threshold: " << min_rtt_inc_threshold
                                << " start_found: " << m_start_found << " cong_win: " << cong_win;
        }
        return (cong_win >= START_LOW_WIN) && m_start_found;
    }
                                          ...
                                          uint64_t delay_min, uint64_t event_time)
    {
        ...
        if (m_epoch == 0) {
            ...
            if (m_last_max_cong_win <= cur_cong_win) {
                ...
                m_time_to_ori_point =
                    (uint64_t)cbrt(float(CubeFactor * (m_last_max_cong_win - cur_cong_win)));
                m_ori_point_cong_win = m_last_max_cong_win;
            }
        }
        int64_t elapsed_time = (int64_t)((event_time + delay_min - m_epoch) << 10 / (1000 * 1000));
        int64_t offset = (int64_t)(m_time_to_ori_point)-elapsed_time;
        if (offset < 0) {
            offset = -offset;
        }
        uint64_t delta_cong_win =
            CubeCongestionWindowScale * offset * offset * offset * MaxDatagramSize >> CubeScale;
        ...
        m_estimated_tcp_cong_win +=
            uint64_t(float(m_acked_bytes_count) * alpha() * float(MaxDatagramSize)
    ...
    {
        if (inSlowStart()
            && m_slow_start.shouldExitSlowStart(m_rtt->latestRTT(), m_rtt->minRTT(),
                                                getCongestionWindow() / m_max_datagram_size)) {
            _LOG_WARN(g_logger) << "trace now: " << GetCurrentUS()
    ...
    CubicSender::CubicSender(uint64_t clock, RTTStats::ptr stats, bool reno,
                             uint64_t initial_max_datagram_size, uint64_t initial_cong_win,
                             uint64_t initial_max_cong_win)
        : m_clock(clock), m_rtt(stats), m_reno(reno),
          m_max_datagram_size(initial_max_datagram_size), m_initial_cong_win(initial_cong_win),
          m_initial_max_cong_win(initial_max_cong_win)
    {
        m_cubic = std::make_shared<Cubic>();
        m_pacer = std::make_shared<Pacer>([this]() -> uint64_t {
            uint64_t srtt = m_rtt->smoothedRTT();
            if (srtt == 0) {
                return ~0ull;
            }
            uint64_t cong_size = this->getCongestionWindow();
            _LOG_WARN(g_logger) << "trace now: " << GetCurrentUS() << " congestion_win "
                                << cong_size;
    ...
    {
        m_pacer = std::make_shared<Pacer>(std::bind(&CubicSender::bwEstimate, shared_from_this()));
    }
    ...
    bool CubicSender::isCwndLimited(uint64_t bytes_inflight)
    {
        ...
        return slow_start_limited || available_bytes <= MaxBurstPackets * m_max_datagram_size;
    }
                                        ...
                                        uint64_t prior_inflight, uint64_t event_time)
    {
        ...
        if (m_reno) { // classic reno congestion avoidance
            ...
            m_cong_win = std::min(maxCongestionWindow(),
                                  m_cubic->congestionWinAfterAck(acked_bytes, m_cong_win,
                                                                 m_rtt->minRTT(), event_time));
        }
    }
                                    ...
                                    uint64_t prior_inflight, uint64_t event_time)
    {
        ...
        _LOG_WARN(g_logger) << "trace now: " << GetCurrentUS() << "  onPacketAcked " << m_cong_win;
        if (inSlowStart()) {
            m_slow_start.onPacketAcked(acked_pn);
        }
    }

    void CubicSender::onPacketSent(uint64_t sent_time, uint64_t bytes_inflight, QuicPacketNumber pn,
    ...
    {
        if (m_largest_sent_at_last_cutback != ~0ull && pn <= m_largest_sent_at_last_cutback) {
            return;
        }
        ...
        m_slow_start_threshold = m_cong_win;
        _LOG_WARN(g_logger) << "trace now: " << GetCurrentUS() << "  onPacketLost " << m_cong_win;
    ...
    void CubicSender::onRetransmissionTimeout(bool packet_retransmitted)
    {
        ...
        _LOG_WARN(g_logger) << "trace now: " << GetCurrentUS() << "  slow_start_threshold2 "
                            << m_slow_start_threshold;
