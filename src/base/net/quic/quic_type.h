...
#include "base/net/streams/async_socket_stream.h"
...
namespace quic
{
    ...
    protected:
        ...
        QuicError(QuicErrorClass error_class, uint16_t error_code, const char *error_msg = nullptr)
            : m_cls(error_class), m_code(error_code), m_msg(error_msg){};
    };
    ...
    public:
        ...
        QuicConnectionError(QuicErrorClass error_class, uint16_t error_code,
                            const char *error_msg = nullptr,
                            QuicFrameType frame_type = QuicFrameType::UNKNOWN)
            : QuicError(error_class, error_code, error_msg), m_frame_type(frame_type){};
    ...
    public:
        ...
        QuicStatelessResetToken(const uint8_t *buf)
        {
            memcpy(this->m_token, buf, QuicStatelessResetToken::LEN);
        }
    ...
    struct AckRange {
        ...
        AckRange(QuicPacketNumber start, QuicPacketNumber end) : m_smallest(start), m_largest(end)
        {
        }
