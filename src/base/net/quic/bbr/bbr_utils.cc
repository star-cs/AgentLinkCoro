...
namespace quic
{
    ...
    WindowedFilter::WindowedFilter(uint64_t win_len) : m_win_length(win_len)
    {
        m_estimates[0] = sample();
        m_estimates[1] = sample();
        m_estimates[2] = sample();
        m_compare_cb = [](uint64_t a, uint64_t b) -> bool { return a >= b; };
    }
    ...
    void WindowedFilter::update(uint64_t new_bw, uint64_t new_time)
    {
        ...
        if (m_estimates[0].bandwidth == 0 || m_compare_cb(new_bw, m_estimates[0].bandwidth)
    ...
    ConnectionStateOnSentPacket::ConnectionStateOnSentPacket(uint64_t sent_time, uint64_t bytes,
                                                             BandwidthSampler *b)
        : sentTime(sent_time), size(bytes), totalBytesSent(b->m_total_bytes_sent),
                                        ...
                                        uint64_t bytes_inflight, bool has_retransmittable_data)
    {
        ...
        if (bytes_inflight == 0) {
            ...
            // In this situation ack compression is not a concern, set send rate to effectively
            // infinite.
            m_last_acked_packet_sent_time = sent_time;
        }
        m_connection_state_map[pn] =
            std::make_shared<ConnectionStateOnSentPacket>(sent_time, bytes, this);
    }
    ...
    BandwidthSample BandwidthSampler::onPacketAcked(uint64_t ack_time, uint64_t pn)
    {
        ...
        if (sent_packet->sentTime < sent_packet->lastAckedPacketSentTime) {
            sent_rate = BandwidthFromDelta(
                sent_packet->totalBytesSent - sent_packet->totalBytesSentAtLastAckedPacket,
        ...
        uint64_t ack_rate = BandwidthFromDelta(
            m_total_bytes_acked - sent_packet->totalBytesAckedAtTheLastAckedPacket,
            ack_time - sent_packet->lastAckedPacketAckTime);

        return BandwidthSample{std::min(sent_rate, ack_rate),
                               // Note: this sample does not account for delayed acknowledgement
                               // time.  This means that the RTT measurements here can be
                               // artificially high, especially on low bandwidth connections.
                               ack_time - sent_packet->sentTime,
                               // A sample is app-limited if the packet was sent during the
                               // app-limited phase.
    ...
    {
        for (auto it = m_connection_state_map.begin(); it != m_connection_state_map.end();) {
