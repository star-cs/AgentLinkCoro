...
namespace quic
{
    ...
    enum class Mode : u_int8_t { Startup = 1, Drain, ProbeBW, ProbeRTT };
    ...
    class BbrSender : public std::enable_shared_from_this<BbrSender>, public SendAlgorithm
    {
    public:
        ...
        BbrSender(RTTStats::ptr rtt, uint64_t initial_max_datagram_size,
                  ...
                  uint64_t initial_max_cong_win = MaxCongestionWindowPackets
                                                  * InitialMaxDatagramSize)
            : m_rtt(rtt), m_max_bw(RoundTripCount), m_initial_cong_win(initial_cong_win),
        ...
        void onPacketAcked(QuicPacketNumber pn, uint64_t acked_bytes, uint64_t prior_inflight,
                           uint64_t event_time) override
        {
        }
        void onPacketSent(uint64_t sent_time, uint64_t bytes_inflight, QuicPacketNumber pn,
                          uint64_t bytes, bool is_retransmittable) override;
        void onPacketLost(QuicPacketNumber pn, uint64_t lost_bytes,
                          uint64_t prior_infligth) override
        {
        }
        void onRetransmissionTimeout(bool packet_retransmitted) override {}
        bool canSend(uint64_t bytes_inflight) override
        {
            return bytes_inflight < getCongestionWin();
        }
        ...
        void onCongEvent(bool rtt_updated, uint64_t prior_inflight, uint64_t bytes_inflight,
