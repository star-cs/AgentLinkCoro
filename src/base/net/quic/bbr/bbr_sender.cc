...
namespace quic
{
    ...
    uint64_t BbrSender::getCongestionWin()
    {
        ...
        _LOG_WARN(g_logger) << "trace now: " << GetCurrentUS() << " BbrSender getCongestionWin3 "
                            << m_cong_win;
        return m_cong_win;
    }
                                 ...
                                 uint64_t bytes, bool is_retransmittable)
    {
        ...
        if (bytes_inflight == 0 && m_sampler.m_is_app_limited) {
    ...
    bool BbrSender::updateBwAndMinRtt(uint64_t now,
                                      const std::vector<QuicPacket::ptr> &acked_packets)
    {
        ...
        for (const auto &packet : acked_packets) {
            ...
            if (!bw_sample.isApplimited || bw_sample.bandwith > bwEstimate()) {
        ...
        if (min_rtt_expired || sample_min_rtt < m_min_rtt || m_min_rtt == 0) {
                                        ...
                                        bool is_round_start)
    {
        ...
        switch (m_recovery_state) {
            ...
            case RecoveryState::Growth: {
                if (!has_losses && last_acked_pn > m_end_recovery_at) {
    ...
    void BbrSender::updateGainCyclePhase(uint64_t now, uint64_t prior_inflight, bool has_losses)
    {
        ...
        if (m_pacing_gain > 1 && !has_losses && prior_inflight < getTargetCongWin(m_pacing_gain)) {
    ...
    {
        if (m_mode == Mode::Startup && m_is_at_full_bw) {
            m_mode = Mode::Drain;
            m_pacing_gain = DrainGain;
            m_cong_win = HighGain;
        }
        if (m_mode == Mode::Drain && bytes_inflight <= getTargetCongWin(1)) {
            enterProbeBwMode(now);
        }
    }

    void BbrSender::maybeEnterOrExitProbeRtt(uint64_t now, bool is_round_start,
                                             bool min_rtt_expired, uint64_t bytes_inflight)
    {
        if (min_rtt_expired && !m_exiting_quiescense && m_mode != Mode::ProbeRTT) {
        ...
        if (m_mode == Mode::ProbeRTT) {
            ...
            if (m_exit_probe_rtt_at == 0) {
                ...
                if (now >= m_exit_probe_rtt_at && m_probe_rtt_round_passed) {
                    m_min_rtt_ts = now;
                    if (!m_is_at_full_bw) {
                        enterStartupMode();
                    } else {
    ...
    void BbrSender::calcCongWin(uint64_t bytes_acked)
    {
        ...
        } else if (m_cong_win < target_win || m_sampler.m_total_bytes_acked < m_initial_cong_win) {
    ...
    void BbrSender::onCongEvent(bool rtt_updated, uint64_t prior_inflight, uint64_t bytes_inflight,
                                uint64_t event_time, std::vector<QuicPacket::ptr> acked_packets,
                                std::vector<QuicPacket::ptr> lost_packets, uint64_t least_unacked)
    {
