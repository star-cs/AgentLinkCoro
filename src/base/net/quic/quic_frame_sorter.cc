...
namespace quic
{
                                             ...
                                             const std::function<void()> &cb)
    {
        ...
        if ((start_equal_end_gap && end <= start_gap->start())
            || (!start_equal_end_gap && start_gap->end() >= end_gap->start()
                && end <= start_gap->start())) {
        ...
        for (;;) {
            ...
            if (end - pos > old_entry_len || (has_replaced && end - pos == old_entry_len)) {
                m_queue.erase(it->first);
                pos += old_entry_len;
                has_replaced = true;
                it->second->cb();
    ...
    {
        std::stringstream ss;
        ss << "FrameSorter queue size: " << m_queue.size() << ", gaps size: " << m_gaps.size()
           << ", read pos: " << m_read_pos << "\n";

        for (auto it = m_queue.begin(); it != m_queue.end(); it++) {
            ss << "[offset: " << it->first << " entry_size: " << it->second->size() << "], ";
        }
        ss << "\n";
        for (auto it = m_gaps.begin(); it != m_gaps.end(); it++) {
            ss << "{start: " << (*it)->start() << " end: " << (*it)->end() << "}, ";
        }
        return ss.str();
    }
} // namespace quic
