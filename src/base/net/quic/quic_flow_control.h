...
namespace quic
{
    ...
    public:
        ...
        // for connection
        BaseFlowController(uint64_t recv_win, uint64_t max_recv_win, const RTTStats::ptr &stat,
                           const std::string &tag = "connection")
            : m_receive_win(recv_win), m_receive_win_size(recv_win),
              m_max_recv_win_size(max_recv_win), m_rtt_stats(stat)
        {
            debug_tag = tag;
        }
        // for stream
        BaseFlowController(uint64_t recv_win, uint64_t max_recv_win, uint64_t initial_win_size,
                           const RTTStats::ptr &stat, const std::string &tag = "stream")
            : m_receive_win(recv_win), m_receive_win_size(recv_win),
              m_max_recv_win_size(max_recv_win), m_send_win(initial_win_size), m_rtt_stats(stat)
        {
            debug_tag = tag;
        }
    ...
    class ConnectionFlowController : public BaseFlowController, public ConnectionFlowControllerI
    {
    public:
        typedef std::shared_ptr<ConnectionFlowController> ptr;
        ConnectionFlowController(uint64_t recv_win, uint64_t max_recv_win,
                                 const RTTStats::ptr &stat,
                                 const std::function<void()> &queue_win_update)
            : BaseFlowController(recv_win, max_recv_win, stat), m_queue_win_update(queue_win_update)
        {
        }
    ...
    public:
        ...
        StreamFlowController(QuicStreamId stream_id, ConnectionFlowController::ptr conn,
                             ...
                             uint64_t recv_win, uint64_t max_recv_win, uint64_t initial_send_win,
                             const RTTStats::ptr &stat)
            : BaseFlowController(recv_win, max_recv_win, initial_send_win, stat),
              m_stream_id(stream_id), m_connection(conn), m_queue_win_update(queue_win_update)
        {
        }
