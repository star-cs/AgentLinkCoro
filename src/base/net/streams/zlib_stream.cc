...
{
    _ASSERT((level >= 0 && level <= 9) || level == DEFAULT_COMPRESSION);
    _ASSERT((window_bits >= 8 && window_bits <= 15));
    _ASSERT((memlevel >= 1 && memlevel <= 9));
...
base::ByteArray::ptr ZlibStream::getByteArray()
{
