...
void AsyncSocketStream::Ctx::doRsp()
{
    ...
    if (!base::Atomic::compareAndSwapBool(scheduler, scd, (Scheduler *)nullptr)) {
        return;
    }
    _LOG_DEBUG(g_logger) << "scd=" << scd << " fiber=" << fiber;
    if (!scd || !fiber) {
        return;
    }
    if (timer) {
...
{
    if (!m_iomanager) {
        m_iomanager = base::IOManager::GetThis();
    }
    if (!m_worker) {
        m_worker = base::IOManager::GetThis();
    }
...
void AsyncSocketStream::doRead()
{
    ...
    _LOG_DEBUG(g_logger) << "doRead out " << this;
    innerClose();
    m_waitSem.notify();
...
void AsyncSocketStream::doWrite()
{
    ...
    _LOG_DEBUG(g_logger) << "doWrite out " << this;
    {
        RWMutexType::WriteLock lock(m_queueMutex);
        m_queue.clear();
    }
...
{
    _LOG_DEBUG(g_logger) << "onTimeOut " << ctx;
    {
...
{
    _ASSERT(m_iomanager == base::IOManager::GetThis());
...
AsyncSocketStream::ptr AsyncSocketStreamManager::get()
{
    ...
    for (uint32_t i = 0; i < m_size; ++i) {
        auto idx = base::Atomic::addFetch(m_idx, 1);
