...
static base::ConfigVar<uint32_t>::ptr g_service_discover_redis_ttl = base::Config::Lookup(
    "service_discovery.redis.ttl", (uint32_t)10, "service discovery redis ttl");
static base::ConfigVar<uint32_t>::ptr g_service_discover_redis_check =
    base::Config::Lookup("service_discovery.redis.check_interval", (uint32_t)3,
                         "service discovery redis check interval");
static base::ConfigVar<uint32_t>::ptr g_service_discover_consul_check =
    base::Config::Lookup("service_discovery.consul.check_interval", (uint32_t)15,
                         "service discovery consul check interval");
                                             ...
                                             const std::string &data)
{
    ...
    auto port = base::TypeUtil::Atoi(ip_and_port.substr(pos + 1));
    ...
    std::vector<std::string> parts = base::split(data, '&');
    for (auto &i : parts) {
        auto pos = i.find('=');
        if (pos == std::string::npos) {
            continue;
...
void ZKServiceDiscovery::start()
{
    ...
    m_client = std::make_shared<base::ZKClient>();
    bool b = m_client->init(m_hosts, 6000,
                            std::bind(&ZKServiceDiscovery::onWatch, self, std::placeholders::_1,
                                      std::placeholders::_2, std::placeholders::_3,
                                      std::placeholders::_4));
    if (!b) {
        _LOG_ERROR(g_logger) << "ZKClient init fail, hosts=" << m_hosts;
    }
...
void ZKServiceDiscovery::onZKConnect(const std::string &path, ZKClient::ptr client)
{
    ...
    if (!ok) {
        _LOG_ERROR(g_logger) << "onZKConnect register fail";
    }
    ...
    if (!ok) {
        _LOG_ERROR(g_logger) << "onZKConnect query fail";
    }
    ...
    if (!ok) {
        _LOG_ERROR(g_logger) << "onZKConnect queryData fail";
    }
}
...
bool ZKServiceDiscovery::existsOrCreate(const std::string &path)
{
    ...
    if (v == ZOK) {
        ...
        if (pos == std::string::npos) {
            _LOG_ERROR(g_logger) << "existsOrCreate invalid path=" << path;
            return false;
        }
                                      ...
                                      const std::string &ip_and_port, const std::string &data)
{
    ...
    if (!v) {
        _LOG_ERROR(g_logger) << "create path=" << path << " fail";
        return false;
    }
    ...
    if (!m_isOnTimer) {
        _LOG_ERROR(g_logger) << "create path=" << (path + "/" + ip_and_port)
...
bool ZKServiceDiscovery::queryInfo(const std::string &domain, const std::string &service)
{
    if (service != "all") {
        ...
        if (m_selfInfo.empty()) {
            _LOG_ERROR(g_logger) << "queryInfo selfInfo is null";
            return false;
        }
        ...
        if (!m_isOnTimer) {
            _LOG_ERROR(g_logger) << "create path=" << (path + "/" + m_selfInfo)
                                 << " fail, error:" << zerror(rt) << " (" << rt << ")";
        }
        return rt == ZNODEEXISTS;
    } else {
...
bool ZKServiceDiscovery::getChildren(const std::string &path)
{
    ...
    if (!ParseDomainService(path, domain, service)) {
        _LOG_ERROR(g_logger) << "get_children path=" << path << " invalid path";
        return false;
    }
    {
        base::RWMutex::ReadLock lock(m_mutex);
        ...
        if (it == m_queryInfos.end()) {
            _LOG_ERROR(g_logger) << "get_children path=" << path << " domian=" << domain
                                 << " not exists";
            return false;
        }
        if (it->second.count(service) == 0 && it->second.count("all") == 0) {
            _LOG_ERROR(g_logger) << "get_children path=" << path << " service=" << service
                                 << " not exists "
                                 << base::Join(it->second.begin(), it->second.end(), ",");
            return false;
        }
    }
    ...
    if (v != ZOK) {
        _LOG_ERROR(g_logger) << "get_children path=" << path << " fail, error:" << zerror(v) << " ("
                             << v << ")";
        return false;
    }
    ...
    for (auto &i : vals) {
        ...
        _LOG_INFO(g_logger) << "domain=" << domain << " service=" << service
                            << " info=" << info->toString();
    }
...
{
    // _LOG_INFO(g_logger) << "query_data domain=" << domain
    //                          << " service=" << service;
    if (service != "all") {
        std::string path = GetProvidersPath(domain, service);
...
void ZKServiceDiscovery::onWatch(int type, int stat, const std::string &path, ZKClient::ptr client)
{
    ...
    _LOG_ERROR(g_logger) << "onWatch hosts=" << m_hosts << " type=" << type << " stat=" << stat
                         << " path=" << path << " client=" << client;
}
#endif

#if WITH_REDIS
RedisServiceDiscovery::RedisServiceDiscovery(const std::string &name) : m_name(name)
{
}
    ...
    for (auto &i : rinfo) {
        ...
        for (auto &n : i.second) {
            ...
            for (auto &v : n.second) {
                ...
                if (!base::RedisUtil::TryCmd(m_name, 5, "hset sylar:%s:%s %s %s", i.first.c_str(),
                                             n.first.c_str(), v.first.c_str(),
                                             MapToStr(m).c_str())) {
                    _LOG_ERROR(g_logger) << "register hset sylar:" << i.first << ":" << n.first
                                         << " " << v.first << " " << MapToStr(m) << " fail";
                }
            }
        }
        if (!i.second.empty()) {
            if (!base::RedisUtil::TryCmd(m_name, 5, ss.str().c_str())) {
                _LOG_ERROR(g_logger) << "register server fail:" << ss.str();
            }
        }
    }
    if (!rinfo.empty()) {
        if (!base::RedisUtil::TryCmd(m_name, 5, hss.str().c_str())) {
            _LOG_ERROR(g_logger) << "register server fail:" << hss.str();
        }
    }
    return true;
}
...
bool RedisServiceDiscovery::queryInfo()
{
    ...
    for (auto &i : infos) {
        ...
        // _LOG_INFO(g_logger) << "==" << i.first << " - " << base::Join(i.second.begin(),
        // i.second.end(), ",");
        if (i.second.count("all")) {
            auto rpy = base::RedisUtil::TryCmd(m_name, 5, "hkeys sylar:%s", i.first.c_str());
            if (!rpy) {
                _LOG_ERROR(g_logger) << "query_info error, hkeys sylar:" << i.first;
                continue;
            }
        ...
        for (auto &n : services) {
            ...
            auto rpy = base::RedisUtil::TryCmd(m_name, 5, "hgetall sylar:%s:%s", i.first.c_str(),
                                               n.c_str());
            if (!rpy) {
                _LOG_ERROR(g_logger) << "query_info error, hgetall sylar:" << i.first << ":" << n;
                continue;
            }
            ...
            for (size_t x = 0; (x + 1) < rpy->elements; x += 2) {
                ...
                if (!info) {
                    _LOG_ERROR(g_logger)
                        << "invalid data format sylar:" << i.first << ":" << n << " "
                        << std::string(rpy->element[x]->str, rpy->element[x]->len);
                    continue;
                }
                // _LOG_INFO(g_logger) << "=" << info->toString();
                if (!info
                    || (now - info->getUpdateTime()) > g_service_discover_redis_ttl->getValue()) {
                    // _LOG_INFO(g_logger) << "*" << info->toString();
                    continue;
                }
...
void RedisServiceDiscovery::start()
{
    ...
    if (!m_timer) {
        m_timer = base::IOManager::GetThis()->addTimer(
            g_service_discover_redis_check->getValue() * 1000,
            [this]() {
                if (m_startRegister) {
                    registerSelf();
                }
...
ConsulServiceDiscovery::ConsulServiceDiscovery(const std::string &name,
                                               base::ConsulClient::ptr client,
                                               base::ConsulRegisterInfo::ptr regInfo)
    : m_name(name), m_client(client), m_regInfo(regInfo)
{
}
...
void ConsulServiceDiscovery::start()
{
    ...
    if (!m_timer) {
        m_timer = base::IOManager::GetThis()->addTimer(
            g_service_discover_consul_check->getValue() * 1000,
            [this]() {
                if (m_startRegister) {
                    registerSelf();
                }
...
bool ConsulServiceDiscovery::registerSelf()
{
    ...
    if (!v) {
        _LOG_ERROR(g_logger) << "consul register fail, "
                             << base::pack::EncodeToJsonString(m_regInfo, 0);
    }
    return v;
}
...
bool ConsulServiceDiscovery::queryInfo()
{
    ...
    for (auto &i : infos) {
        ...
        if (!m_client->serviceQuery(i.second, cinfos)) {
            _LOG_ERROR(g_logger) << "consul query service fail, "
                                 << base::Join(i.second.begin(), i.second.end(), ",");
            return false;
        }
        std::unordered_map<uint64_t, ServiceItemInfo::ptr> sinfos;
        for (auto &it : cinfos) {
            for (auto &v : it.second) {
                auto info = ServiceItemInfo::Create(v->ip + ":" + std::to_string(v->port),
                                                    base::MapJoin(v->tags.begin(), v->tags.end()));
                if (!info) {
                    _LOG_ERROR(g_logger) << "invalid data format " << it.first << " : "
                                         << base::pack::EncodeToJsonString(v, 0);
                    continue;
                }
                sinfos[info->getId()] = info;
            }
