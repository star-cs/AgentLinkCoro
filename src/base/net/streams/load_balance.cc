...
void LoadBalanceItem::close()
{
    if (m_stream) {
        ...
        base::WorkerMgr::GetInstance()->schedule("service_io", [stream]() { stream->close(); });
    }
}
...
std::string LoadBalanceItem::toString()
{
    ...
    ss << "[Item id=" << m_id << " weight=" << getWeight()
       << " discovery_time=" << base::Time2Str(m_discoveryTime);
    if (!m_stream) {
        ss << " stream=null";
    } else {
...
std::string LoadBalance::statusString(const std::string &prefix)
{
    ...
    lock.unlock();
    std::stringstream ss;
    ss << prefix << "init_time: " << base::Time2Str(m_lastInitTime / 1000) << std::endl;
    for (auto &i : datas) {
...
{
    // _LOG_INFO(g_logger) << "onServiceChange domain=" << domain
    //                          << " service=" << service;
    auto type = getType(domain, service);
    ...
    for (auto &i : add_values) {
        // _LOG_INFO(g_logger) << "*** " << i.second->getType();
        // if(i.second->getType() != m_type) {
        //     continue;
        // }
        auto stream = m_cb(domain, service, i.second);
        if (!stream) {
            _LOG_ERROR(g_logger) << "create stream fail, " << i.second->toString();
            continue;
        }
...
void SDLoadBalance::start()
{
    ...
    m_timer =
        base::IOManager::GetThis()->addTimer(500, std::bind(&SDLoadBalance::refresh, this), true);
    m_sd->start();
}
