...
namespace http
{
    ...
    {
        _LOG_DEBUG(g_logger) << "HttpConnection::~HttpConnection";
    }
    ...
    HttpResponse::ptr HttpConnection::recvResponse()
    {
        ...
        do {
            ...
            // _LOG_INFO(g_logger) << "read len=" << len;
            if (len <= 0) {
                close();
                // _LOG_ERROR(g_logger) << "recv_response len <= 0 errno=" << errno << "
            ...
            if (parser->hasError()) {
                close();
                // _LOG_ERROR(g_logger) << "recv_response parser has error errno=" << errno <<
                // " errstr=" << strerror(errno)
                //     << " [" << all_data << "]";
                return nullptr;
            }
            ...
            if (offset == (int)buff_size) {
                close();
                // _LOG_ERROR(g_logger) << "recv_response offset == buff_size errno=" << errno
                // << " errstr=" << strerror(errno)
                //     << " [" << all_data << "]";
                return nullptr;
            }
        ...
        // _LOG_INFO(g_logger) << "after header";
        auto &client_parser = parser->getParser();
        std::string body;
        if (client_parser.chunked) {
            // _LOG_INFO(g_logger) << "chunked --- " << *parser->getData();
            int len = offset;
            do {
                bool begin = true;
                do {
                    if (!begin || len == 0) {
                        ...
                        if (rt <= 0) {
                            close();
                            // _LOG_ERROR(g_logger) << "recv_response rt <= 0 " << rt << "
                    ...
                    if (parser->hasError()) {
                        goto out;
                        // _LOG_ERROR(g_logger) << "recv_response parser has error errno=" <<
                        // errno << " errstr=" << strerror(errno)
                        //     << " offset=" << offset << " len=" << len << " begin=" << begin << "
                        //     - finished=" << parser->isFinished() << " - nparse=" << nparse
                        //     << " [" << all_data << "]";
                        close();
                        return nullptr;
                    }
                    ...
                    if (len == (int)buff_size) {
                        close();
                        // _LOG_ERROR(g_logger) << "recv_response len == buff_size errno=" <<
                        // errno << " errstr=" << strerror(errno)
                        //     << " [" << all_data << "]";
                        return nullptr;
                    }
                ...
                _LOG_DEBUG(g_logger) << "content_len=" << client_parser.content_len;
            ...
            if (length > 0) {
                ...
                if (length > 0) {
                    if (readFixSize(&body[len], length) <= 0) {
                        close();
                        // _LOG_ERROR(g_logger) << "recv_response readFixSize error errno=" <<
                        // errno << " errstr=" << strerror(errno)
                        //         << " [" << all_data << "]";
                        return nullptr;
                    }
    ...
    out:
        if (!body.empty()) {
            ...
            _LOG_DEBUG(g_logger) << "content_encoding: " << content_encoding
                                 << " size=" << body.size();
                                              ...
                                              uint64_t timeout_ms)
    {
        ...
        // }
        // uint64_t ts1 = base::GetCurrentMS();
        ...
        // }
        // timeout_ms -= base::GetCurrentMS() - ts1;
    ...
    HttpResult::ptr HttpConnection::DoRequest(HttpRequest::ptr req, Address::ptr addr, bool is_ssl,
                                              uint64_t timeout_ms)
    {
        Socket::ptr sock = is_ssl ? SSLSocket::CreateTCP(addr) : Socket::CreateTCP(addr);
        if (!sock) {
    ...
    HttpResult::ptr HttpConnection::DoRequest(HttpRequest::ptr req, Socket::ptr sock,
                                              uint64_t timeout_ms)
    {
        sock->setRecvTimeout(timeout_ms);
        HttpConnection::ptr conn = std::make_shared<HttpConnection>(sock);
                                                       ...
                                                       uint32_t max_request)
    {
        ...
        if (!turi) {
            _LOG_ERROR(g_logger) << "invalid uri=" << uri;
        }
    ...
    {
        uint64_t now_ms = base::GetCurrentMS();
        ...
        if (!ptr) {
            ...
            if (!addr) {
                _LOG_ERROR(g_logger) << "get addr fail: " << m_host;
                return nullptr;
            }
            // addr->setPort(m_port);
            Socket::ptr sock = m_isHttps ? SSLSocket::CreateTCP(addr) : Socket::CreateTCP(addr);
            if (!sock) {
                _LOG_ERROR(g_logger) << "create sock fail: " << *addr;
                return nullptr;
            }
            uint64_t ts1 = base::GetCurrentMS();
            if (!sock->connect(addr, timeout_ms)) {
                _LOG_ERROR(g_logger) << "sock connect fail: " << *addr;
                return nullptr;
            }
            timeout_ms -= base::GetCurrentMS() - ts1;
    ...
    {
        ++ptr->m_request;
        if (pool->m_total >= (int32_t)pool->m_maxSize || !ptr->isConnected()
            || (ptr->m_request >= pool->m_maxRequest)
            || ((ptr->m_createTime + pool->m_maxAliveTime) >= base::GetCurrentMS())
            || !ptr->checkConnected()) {
            delete ptr;
            --pool->m_total;
            return;
