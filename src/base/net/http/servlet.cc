...
namespace http
{
    ...
    int32_t FunctionServlet::handle(base::http::HttpRequest::ptr request,
    ...
    int32_t ServletDispatch::handle(base::http::HttpRequest::ptr request,
                                    base::http::HttpResponse::ptr response,
                                    base::SocketStream::ptr session)
    {
    ...
    void
    ServletDispatch::listAllGlobServletCreator(std::map<std::string, IServletCreator::ptr> &infos)
    {
        RWMutexType::ReadLock lock(m_mutex);
        for (auto &i : m_globs) {
            infos[i.first] = i.second;
    ...
    int32_t NotFoundServlet::handle(base::http::HttpRequest::ptr request,
                                    base::http::HttpResponse::ptr response,
                                    base::SocketStream::ptr session)
    {
        response->setStatus(base::http::HttpStatus::NOT_FOUND);
