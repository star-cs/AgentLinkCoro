...
namespace http
{
    ...
    struct _ProfilerStatus {
        ...
        _PACK(O(cpu_profiler, mem_profiler, cpu_start_time, mem_start_time, cpu_profiler_file,
                mem_profiler_file));
    };
    ...
    int32_t handleStatus(base::http::HttpRequest::ptr request,
                         base::http::HttpResponse::ptr response, base::SocketStream::ptr session)
    {
        response->setBody(base::pack::EncodeToJsonString(s_status, 0));
        return 0;
    }

    int32_t handleStart(base::http::HttpRequest::ptr request,
                        base::http::HttpResponse::ptr response, base::SocketStream::ptr session)
    {
        ...
        if (type == "cpu") {
            if (s_status.cpu_profiler) {
                ...
                s_status.cpu_start_time = base::GetCurrentMS();
            ...
            if (s_status.mem_profiler) {
                ...
                s_status.mem_start_time = base::GetCurrentMS();
        ...
        response->setBody(base::pack::EncodeToJsonString(rsp, 0));
        return 0;
    }

    int32_t handleStop(base::http::HttpRequest::ptr request,
                       base::http::HttpResponse::ptr response, base::SocketStream::ptr session)
    {
        auto type = request->getParam("type");
        _RspStruct rsp;
    ...
    int32_t ProfilerServlet::handle(base::http::HttpRequest::ptr request,
