...
namespace http
{
    ...
    base::ConfigVar<uint32_t>::ptr g_websocket_message_max_size = base::Config::Lookup(
    ...
    HttpRequest::ptr WSSession::handleShake()
    {
        ...
        do {
            req = recvRequest();
            if (!req) {
                _LOG_INFO(g_logger) << "invalid http request";
                break;
            }
            if (strcasecmp(req->getHeader("Upgrade").c_str(), "websocket")) {
                _LOG_INFO(g_logger) << "http header Upgrade != websocket";
                break;
            }
            if (strcasecmp(req->getHeader("Connection").c_str(), "Upgrade")) {
                _LOG_INFO(g_logger) << "http header Connection != Upgrade";
                break;
            }
            if (req->getHeaderAs<int>("Sec-webSocket-Version") != 13) {
                _LOG_INFO(g_logger) << "http header Sec-webSocket-Version != 13";
                break;
            }
            std::string key = req->getHeader("Sec-WebSocket-Key");
            if (key.empty()) {
                _LOG_INFO(g_logger) << "http header Sec-WebSocket-Key = null";
                break;
            }

            std::string v = key + "258EAFA5-E914-47DA-95CA-C5AB0DC85B11";
            v = base::base64encode(base::sha1sum(v));
            ...
            sendResponse(rsp);
            _LOG_DEBUG(g_logger) << *req;
            _LOG_DEBUG(g_logger) << *rsp;
            return req;
        } while (false);
        if (req) {
            _LOG_INFO(g_logger) << *req;
        }
        return nullptr;
    }
    ...
    WSFrameMessage::ptr WSRecvMessage(Stream *stream, bool client)
    {
        ...
        do {
            ...
            _LOG_DEBUG(g_logger) << "WSFrameHead " << ws_head.toString();

            if (ws_head.opcode == WSFrameHead::PING) {
                _LOG_INFO(g_logger) << "PING";
                if (WSPong(stream) <= 0) {
                    break;
                }
                ...
                if (!client && !ws_head.mask) {
                    _LOG_INFO(g_logger) << "WSFrameHead mask != 1";
                    break;
                }
                uint64_t length = 0;
                ...
                if (ws_head.fin) {
                    _LOG_DEBUG(g_logger) << data;
                    return std::make_shared<WSFrameMessage>(opcode, std::move(data));
                }
            } else {
                _LOG_DEBUG(g_logger) << "invalid opcode=" << ws_head.opcode;
            }
        } while (true);
        stream->close();
        return nullptr;
