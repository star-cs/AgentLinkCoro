...
namespace http
{
    ...
    static base::ConfigVar<uint64_t>::ptr g_http_request_buffer_size = base::Config::Lookup(
        "http.request.buffer_size", (uint64_t)(4 * 1024), "http request buffer size");

    static base::ConfigVar<uint64_t>::ptr g_http_request_max_body_size = base::Config::Lookup(
        "http.request.max_body_size", (uint64_t)(64 * 1024 * 1024), "http request max body size");

    static base::ConfigVar<uint64_t>::ptr g_http_response_buffer_size = base::Config::Lookup(
        "http.response.buffer_size", (uint64_t)(4 * 1024), "http response buffer size");

    static base::ConfigVar<uint64_t>::ptr g_http_response_max_body_size = base::Config::Lookup(
    ...
    void on_request_method(void *data, const char *at, size_t length)
    {
        ...
        if (m == HttpMethod::INVALID_METHOD) {
            _LOG_WARN(g_logger) << "invalid http request method: " << std::string(at, length);
    ...
    {
        // _LOG_INFO(g_logger) << "on_request_fragment:" << std::string(at, length);
        HttpRequestParser *parser = static_cast<HttpRequestParser *>(data);
    ...
    void on_request_version(void *data, const char *at, size_t length)
    {
        ...
        if (strncmp(at, "HTTP/1.1", length) == 0) {
            ...
            _LOG_WARN(g_logger) << "invalid http request version: " << std::string(at, length);
                               ...
                               size_t vlen)
    {
        HttpRequestParser *parser = static_cast<HttpRequestParser *>(data);
        if (flen == 0) {
            _LOG_WARN(g_logger) << "invalid http request field length == 0";
    ...
    {
        m_data = std::make_shared<base::http::HttpRequest>();
    ...
    void on_response_version(void *data, const char *at, size_t length)
    {
        ...
        if (strncmp(at, "HTTP/1.1", length) == 0) {
            ...
            _LOG_WARN(g_logger) << "invalid http response version: " << std::string(at, length);
            parser->setError(1001);
            return;
        }
                                ...
                                size_t vlen)
    {
        HttpResponseParser *parser = static_cast<HttpResponseParser *>(data);
        if (flen == 0) {
            _LOG_WARN(g_logger) << "invalid http response field length == 0";
            // parser->setError(1002);
            return;
        }
        parser->getData()->setHeader(std::string(field, flen), std::string(value, vlen));
    ...
    {
        m_data = std::make_shared<base::http::HttpResponse>();
