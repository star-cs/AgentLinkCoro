...
namespace http
{
    ...
    FunctionWSServlet::FunctionWSServlet(callback cb, on_connect_cb connect_cb,
        ...
        : WSServlet("FunctionWSServlet"), m_callback(cb), m_onConnect(connect_cb),
          m_onClose(close_cb)
    {
    }

    int32_t FunctionWSServlet::onConnect(base::http::HttpRequest::ptr header,
    ...
    int32_t FunctionWSServlet::onClose(base::http::HttpRequest::ptr header,
                                       base::http::WSSession::ptr session)
    {
        if (m_onClose) {
    ...
    int32_t FunctionWSServlet::handle(base::http::HttpRequest::ptr header,
