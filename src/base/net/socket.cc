...
#include "base/coro/fd_manager.h"
...
Socket::ptr Socket::CreateTCP(base::Address::ptr address)
{
    return std::make_shared<Socket>(address->getFamily(), TCP, 0);
}

Socket::ptr Socket::CreateUDP(base::Address::ptr address)
{
...
bool Socket::getOption(int level, int option, void *result, socklen_t *len)
{
    ...
    if (rt) {
        _LOG_DEBUG(g_logger) << "getOption sock=" << m_sock << " level=" << level
...
bool Socket::setOption(int level, int option, const void *result, socklen_t len)
{
    if (setsockopt(m_sock, level, option, result, (socklen_t)len)) {
        _LOG_DEBUG(g_logger) << "setOption sock=" << m_sock << " level=" << level
                             << " option=" << option << " errno=" << errno
                             << " errstr=" << strerror(errno);
        return false;
    }
...
Socket::ptr Socket::accept()
{
    ...
    if (newsock == -1) {
        _LOG_ERROR(g_logger) << "accept(" << m_sock << ") errno=" << errno
                             << " errstr=" << strerror(errno);
        return nullptr;
    }
...
bool Socket::bind(const Address::ptr addr)
{
    ...
    if (!isValid()) {
        newSock();
        if (_UNLIKELY(!isValid())) {
            return false;
        }
    }

    if (_UNLIKELY(addr->getFamily() != m_family)) {
        _LOG_ERROR(g_logger) << "bind sock.family(" << m_family << ") addr.family("
    ...
    if (uaddr) {
        ...
        if (sock->connect(uaddr)) {
            ...
            base::FSUtil::Unlink(uaddr->getPath(), true);
        }
    }

    if (::bind(m_sock, addr->getAddr(), addr->getAddrLen())) {
        _LOG_ERROR(g_logger) << "bind error errrno=" << errno << " errstr=" << strerror(errno);
        return false;
    }
    getLocalAddress();
    return true;
...
{
    if (!m_remoteAddress) {
        _LOG_ERROR(g_logger) << "reconnect m_remoteAddress is null";
        return false;
    }
...
bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms)
{
    ...
    if (_UNLIKELY(addr->getFamily() != m_family)) {
        _LOG_ERROR(g_logger) << "connect sock.family(" << m_family << ") addr.family("
                             << addr->getFamily() << ") not equal, addr=" << addr->toString();
        return false;
    }
    ...
    if (timeout_ms == (uint64_t)-1) {
        if (::connect(m_sock, addr->getAddr(), addr->getAddrLen())) {
            _LOG_ERROR(g_logger) << "sock=" << m_sock << " connect(" << addr->toString()
...
{
    if (!isValid()) {
        _LOG_ERROR(g_logger) << "listen error sock=-1";
        return false;
    }
    if (::listen(m_sock, backlog)) {
        _LOG_ERROR(g_logger) << "listen error errno=" << errno << " errstr=" << strerror(errno);
        return false;
    }
    return true;
}
...
Address::ptr Socket::getRemoteAddress()
{
    ...
    if (getpeername(m_sock, result->getAddr(), &addrlen)) {
        // _LOG_ERROR(g_logger) << "getpeername error sock=" << m_sock
...
Address::ptr Socket::getLocalAddress()
{
    ...
    if (getsockname(m_sock, result->getAddr(), &addrlen)) {
        _LOG_ERROR(g_logger) << "getsockname error sock=" << m_sock << " errno=" << errno
                             << " errstr=" << strerror(errno);
        return std::make_shared<UnknownAddress>(m_family);
    }
    if (m_family == AF_UNIX) {
...
{
    return IOManager::GetThis()->cancelEvent(m_sock, base::IOManager::READ);
}

bool Socket::cancelWrite()
{
    return IOManager::GetThis()->cancelEvent(m_sock, base::IOManager::WRITE);
}
...
void Socket::newSock()
{
    ...
    if (_LIKELY(m_sock != -1)) {
        ...
        _LOG_ERROR(g_logger) << "socket(" << m_family << ", " << m_type << ", " << m_protocol
                             << ") errno=" << errno << " errstr=" << strerror(errno);
    }
}
...
bool SSLSocket::init(int sock)
{
    ...
    if (v) {
        ...
        if (!v) {
            ...
            _LOG_ERROR(g_logger) << "SSL_accept rt=" << rt << " err=" << e
...
bool SSLSocket::loadCertificates(const std::string &cert_file, const std::string &key_file)
{
    ...
    if (SSL_CTX_use_certificate_chain_file(m_ctx.get(), cert_file.c_str()) != 1) {
        _LOG_ERROR(g_logger) << "SSL_CTX_use_certificate_chain_file(" << cert_file << ") error";
        return false;
    }
    if (SSL_CTX_use_PrivateKey_file(m_ctx.get(), key_file.c_str(), SSL_FILETYPE_PEM) != 1) {
        _LOG_ERROR(g_logger) << "SSL_CTX_use_PrivateKey_file(" << key_file << ") error";
        return false;
    }
    if (SSL_CTX_check_private_key(m_ctx.get()) != 1) {
        _LOG_ERROR(g_logger) << "SSL_CTX_check_private_key cert_file=" << cert_file
                             << " key_file=" << key_file;
        return false;
    }
    return true;
...
SSLSocket::ptr SSLSocket::CreateTCP(base::Address::ptr address)
{
