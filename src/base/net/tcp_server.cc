...
static base::ConfigVar<uint64_t>::ptr g_tcp_server_read_timeout = base::Config::Lookup(
    "tcp_server.read_timeout", (uint64_t)(60 * 1000 * 2), "tcp server read timeout");

static base::Logger::ptr g_logger = _LOG_NAME("system");

TcpServer::TcpServer(base::IOManager *worker, base::IOManager *io_worker,
                     base::IOManager *accept_worker)
    : m_worker(worker), m_ioWorker(io_worker), m_acceptWorker(accept_worker),
      m_recvTimeout(g_tcp_server_read_timeout->getValue()), m_name("sylar/1.0.0"), m_isStop(true)
{
}
...
bool TcpServer::bind(base::Address::ptr addr, bool ssl)
{
                     ...
                     bool ssl)
{
    ...
    for (auto &addr : addrs) {
        ...
        if (!sock->listen()) {
            _LOG_ERROR(g_logger) << "listen fail errno=" << errno << " errstr=" << strerror(errno)
                                 << " addr=[" << addr->toString() << "]";
            fails.push_back(addr);
            continue;
        }
    ...
    for (auto &i : m_socks) {
        _LOG_INFO(g_logger) << "type=" << m_type << " name=" << m_name << " ssl=" << m_ssl
...
void TcpServer::startAccept(Socket::ptr sock)
{
    while (!m_isStop) {
        ...
        if (client) {
            ...
            _LOG_ERROR(g_logger) << "accept errno=" << sock->getError()
...
{
    _LOG_INFO(g_logger) << "handleClient: " << *client;
}
