...
public:
    DnsDefine operator()(const std::string &v)
    {
        ...
        if (!n["domain"].IsDefined()) {
            //_LOG_ERROR(g_logger) << "dns domain is null, " << n;
            return DnsDefine();
        }
        dd.domain = n["domain"].as<std::string>();
        if (!n["type"].IsDefined()) {
            //_LOG_ERROR(g_logger) << "dns type is null, " << n;
            return DnsDefine();
        }
...
static base::ConfigVar<std::set<DnsDefine> >::ptr g_dns_defines =
    base::Config::Lookup("dns.config", std::set<DnsDefine>(), "dns config");
            ...
            [](const std::set<DnsDefine> &old_value, const std::set<DnsDefine> &new_value) {
                for (auto &n : new_value) {
                    if (n.type == Dns::TYPE_DOMAIN) {
                        ...
                        _LOG_ERROR(g_logger) << "invalid type=" << n.type << " domain=" << n.domain;
                    }
                }
            });
    }
...
void Dns::init()
{
    if (m_type != TYPE_ADDRESS) {
        _LOG_ERROR(g_logger) << m_domain << " invalid type " << m_type;
        return;
    }
    ...
    for (auto &i : addrs) {
        if (!base::Address::Lookup(result, i, base::Socket::IPv4, base::Socket::TCP)) {
            _LOG_ERROR(g_logger) << m_domain << " invalid address: " << i;
        }
    }
    initAddress(result);
}

base::Address::ptr Dns::get(uint32_t seed)
{
    if (seed == (uint32_t)-1) {
        seed = base::Atomic::addFetch(m_idx);
    }
...
base::Socket::ptr Dns::getSock(uint32_t seed)
{
    if (seed == (uint32_t)-1) {
        seed = base::Atomic::addFetch(m_idx);
    }
...
std::string Dns::AddressItem::toString()
{
    ...
    ss << *addr << ":" << valid;
    if (pool_size > 0) {
        base::Spinlock::Lock lock(m_mutex);
        ss << ":" << socks.size();
    }
    return ss.str();
}
...
bool Dns::AddressItem::checkValid(uint32_t timeout_ms)
{
    ...
    base::Socket *sock = new base::Socket(addr->getFamily(), base::Socket::TCP, 0);
    valid = sock->connect(addr, timeout_ms);

    if (valid) {
        if (!check_path.empty()) {
            base::http::HttpRequest::ptr req = std::make_shared<base::http::HttpRequest>();
            req->setPath(check_path);
            req->setHeader("host", addr->toString());
            base::Socket::ptr sock_ptr(sock, base::nop<base::Socket>);
            auto rt = base::http::HttpConnection::DoRequest(req, sock_ptr, timeout_ms);
...
void Dns::refresh()
{
    if (m_type == TYPE_DOMAIN) {
        std::vector<Address::ptr> result;
        if (!base::Address::Lookup(result, m_domain, base::Socket::IPv4, base::Socket::TCP)) {
            _LOG_ERROR(g_logger) << m_domain << " invalid address: " << m_domain;
        }
        initAddress(result);
    } else {
        init();
...
base::Address::ptr DnsManager::getAddress(const std::string &service, bool cache, uint32_t seed)
{
    ...
    if (cache) {
        base::IOManager::GetThis()->schedule([service, this]() {
    ...
    return base::Address::LookupAny(service, base::Socket::IPv4, base::Socket::TCP);
}

base::Socket::ptr DnsManager::getSocket(const std::string &service, bool cache, uint32_t seed)
{
    auto dns = get(service);
    if (dns) {
    ...
    auto addr = base::Address::LookupAny(service, base::Socket::IPv4, base::Socket::TCP);
...
void DnsManager::start()
{
    ...
    m_timer = base::IOManager::GetThis()->addTimer(1000, std::bind(&DnsManager::init, this), true);
}
...
std::ostream &DnsManager::dump(std::ostream &os)
{
    ...
    os << "[DnsManager has_timer=" << (m_timer != nullptr)
       << " last_update_time=" << base::Time2Str(m_lastUpdateTime) << " dns.size=" << m_dns.size()
       << "]" << std::endl;
    for (auto &i : m_dns) {
