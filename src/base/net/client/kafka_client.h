#ifndef ___KAFKA_CLIENT_H__
#define ___KAFKA_CLIENT_H__
...
private:
    struct Msg {
        ...
        Msg(const std::string &_topic, const std::string &_msg, const std::string &_key)
            : topic(_topic), msg(_msg), key(_key)
        {
        }
...
private:
    ...
    std::vector<base::Spinlock *> m_mutexs;
};
...
public:
    ...
    void poll(uint64_t t = 0);
    void start(const std::string &topic_name, int32_t partition = RdKafka::Topic::PARTITION_UA,
