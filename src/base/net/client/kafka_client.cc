    ...
    void event_cb(RdKafka::Event &event)
    {
        switch (event.type()) {
            case RdKafka::Event::EVENT_ERROR:
                _LOG_ERROR(g_logger)
                    << "Cb:ERROR (" << RdKafka::err2str(event.err()) << "): " << event.str();
                if (event.err() == RdKafka::ERR__ALL_BROKERS_DOWN) {
                    m_connected = false;
                }
                break;
            case RdKafka::Event::EVENT_STATS:
                _LOG_ERROR(g_logger) << "Cb:\"STATS\": " << event.str();
                break;
            case RdKafka::Event::EVENT_LOG:
                _LOG_ERROR(g_logger)
                    << "Cb:LOG-" << event.severity() << "-" << event.fac() << ": " << event.str();
                break;
            default:
                _LOG_ERROR(g_logger)
                    << "Cb:EVENT " << event.type() << " (" << RdKafka::err2str(event.err())
                    << "): " << event.str() << std::endl;
                break;
        }
    }
...
public:
    ...
    {
        if (message.err()) {
            _LOG_ERROR(g_logger) << "Message delivery for (" << message.len()
                                 << " bytes): " << message.errstr()
                                 << (message.key() ? (" Key: " + *(message.key())) : "");
            m_connected = false;
        } else {
            base::Atomic::addFetch(m_total, 1);
            m_connected = true;
        }
    }
...
{
    return base::Atomic::addFetch(m_count, 1);
}

KafkaProducer::KafkaProducer()
    : m_connected(false), m_eventCb(NULL), m_drCb(NULL), m_totalMsg(0), m_sucessMsg(0)
{
...
bool KafkaProducer::setBrokerList(const std::string &bl)
{
    ...
    if (rt != RdKafka::Conf::CONF_OK) {
        _LOG_ERROR(g_logger) << "set brokerlist fail: " << bl << " " << rt << ":" << m_err;
        return false;
    }
    m_producer.reset(RdKafka::Producer::create(m_conf.get(), m_err));
    if (!m_producer) {
        _LOG_ERROR(g_logger) << "create producer fail: " << m_err;
        return false;
    }
    return true;
}
...
KafkaTopic::ptr KafkaProducer::getTopic(const std::string &topic)
{
    ...
    if (!t) {
        _LOG_ERROR(g_logger) << "create topic fail: " << topic << " " << m_err;
        return nullptr;
    }
...
bool KafkaProducer::produce(const std::string &topic, const std::string &msg)
{
    ...
    base::Atomic::addFetch(m_totalMsg, 1);

    t->incCount();
    RdKafka::ErrorCode resp = m_producer->produce(
        t->getTopic().get(), RdKafka::Topic::PARTITION_UA, RdKafka::Producer::RK_MSG_COPY,
        const_cast<char *>(msg.c_str()), msg.size(), NULL, NULL);
    if (resp != RdKafka::ERR_NO_ERROR) {
        _LOG_ERROR(g_logger) << "Produce failed: " << RdKafka::err2str(resp);
    } else {
        _LOG_DEBUG(g_logger) << "Produced message (" << msg.size() << ") " << msg;
    }
    m_producer->poll(0);
    return true;
}
                            ...
                            const std::string &key)
{
    ...
    RdKafka::ErrorCode resp = m_producer->produce(
        t->getTopic().get(), RdKafka::Topic::PARTITION_UA, RdKafka::Producer::RK_MSG_COPY,
        const_cast<char *>(msg.c_str()), msg.size(), &key, NULL);
    if (resp != RdKafka::ERR_NO_ERROR) {
        _LOG_ERROR(g_logger) << "Produce failed: " << RdKafka::err2str(resp);
    } else {
        _LOG_DEBUG(g_logger) << "Produced message (" << msg.size() << ") " << msg;
...
KafkaConsumer::KafkaConsumer()
    : m_connected(false), m_running(false), m_ccb(true), m_eventCb(NULL), m_totalMsg(0),
      m_sucessMsg(0), m_partition(0), m_startOffset(0), m_curOffset(0)
{
...
bool KafkaConsumer::setBrokerList(const std::string &bl)
{
    ...
    if (!m_consumer) {
        _LOG_ERROR(g_logger) << "create consumer fail: " << m_err;
        return false;
    }
    return true;
}
...
void KafkaConsumer::start(const std::string &topic_name, int32_t partition, int64_t start_offset,
                          const std::string &offset_path)
{
    ...
    if (!tc) {
        _LOG_ERROR(g_logger) << "create topic fail: " << topic_name << " " << m_err;
        return;
    }
    ...
    do {
        ...
        if (resp != RdKafka::ERR_NO_ERROR) {
            _LOG_ERROR(g_logger) << "Failed to start consumer: " << RdKafka::err2str(resp)
                                 << std::endl;
            sleep(2);
        } else {
            break;
...
void KafkaConsumer::updateOffset(const RdKafka::Message &msg)
{
    do {
        ...
        if (it != m_partitionOffsets.end()) {
            base::Atomic::compareAndSwap(it->second, it->second, msg.offset());
            break;
        }
        it = m_partitionTotal.find(msg.partition());
        if (it != m_partitionTotal.end()) {
            base::Atomic::addFetch(it->second);
        }
    } while (0);

    base::RWMutex::WriteLock lock(m_mutex);
    base::Atomic::compareAndSwap(m_partitionOffsets[msg.partition()],
                                 m_partitionOffsets[msg.partition()], msg.offset());
    base::Atomic::addFetch(m_partitionTotal[msg.partition()]);
}
...
KafkaConsumerGroup::KafkaConsumerGroup()
    : m_partitionStart(0), m_partitionEnd(0), m_partitionShare(10),
      m_defaultOffset(RD_KAFKA_OFFSET_END), m_snapInterval(60)
{
    static base::ConfigVar<std::string>::ptr g_system_path =
        base::Config::Lookup<std::string>("system.path");
...
void KafkaConsumerGroup::start()
{
    if (m_offsets.empty()) {
        ...
        std::ifstream ifs;
        if (base::FSUtil::OpenForRead(ifs, file)) {
            std::string line;
            while (std::getline(ifs, line)) {
                ...
                m_offsets[base::TypeUtil::Atoi(line.substr(0, pos))] =
                    base::TypeUtil::Atoi(line.substr(pos + 1));
            }
        }
    }
    int c = ceil((m_partitionEnd - m_partitionStart) * 1.0 / m_partitionShare);
    m_datas.resize(c);
    _LOG_INFO(g_logger) << "m_thread_name=" << m_threadName
    ...
    for (uint32_t i = m_partitionStart; i < m_partitionEnd; ++i) {
        ...
        auto kc = m_datas[idx];
        if (!kc) {
            kc.reset(new base::KafkaConsumer);
            kc->setCcb(false);
            kc->setBrokerList(m_brokerList);
            kc->setCallback(m_cb);
            m_datas[idx] = kc;
        }
        base::FoxThreadMgr::GetInstance()->dispatch(m_threadName, [this, i, kc]() {
            _LOG_INFO(g_logger) << "start consumer topic=" << m_topic << " partition=" << i;
    ...
    m_timer = base::IOManager::GetThis()->addTimer(
        ...
        [this]() {
            auto file = m_snapFilePath + "/" + m_topic + ".tpc";
            std::ofstream ofs;
            if (base::FSUtil::OpenForWrite(ofs, file)) {
                ...
                _LOG_ERROR(g_logger) << "open file error: " << file;
            }
        },
        true);
}
...
{
    os << "[KafkaConsumerGroup topic=" << m_topic << " partition=[" << m_partitionStart << ", "
       << m_partitionEnd << ") total=" << getTotalMsg() << " partition_share=" << m_partitionShare
...
void KafkaProducerGroup::start()
{
    for (int32_t i = 0; i < m_size; ++i) {
        ...
        m_mutexs.push_back(new base::Spinlock);
    }
    m_datas.resize(m_size);
    for (int32_t i = 0; i < m_size; ++i) {
        base::FoxThreadMgr::GetInstance()->dispatch(m_threadName, [this, i]() {
...
bool KafkaProducerGroup::produce(const std::string &topic, const std::string &msg,
                                 const std::string &key)
{
    auto idx = base::Atomic::addFetch(m_index) % m_size;
    Msg::ptr m = std::make_shared<Msg>(topic, msg, key);

    base::Spinlock::Lock lock(*m_mutexs[idx]);
