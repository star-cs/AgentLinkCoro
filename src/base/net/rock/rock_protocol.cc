...
static base::ConfigVar<uint32_t>::ptr g_rock_protocol_max_length = base::Config::Lookup(
    "rock.protocol.max_length", (uint32_t)(1024 * 1024 * 64), "rock protocol max length");

static base::ConfigVar<uint32_t>::ptr g_rock_protocol_gzip_min_length = base::Config::Lookup(
...
bool RockRequest::serializeToByteArray(ByteArray::ptr bytearray)
{
    try {
        ...
        _LOG_ERROR(g_logger) << "RockRequest serializeToByteArray error";
    }
    return false;
}
...
bool RockRequest::parseFromByteArray(ByteArray::ptr bytearray)
{
    try {
        ...
        _LOG_ERROR(g_logger) << "RockRequest parseFromByteArray error " << bytearray->toHexString();
    }
    return false;
}
...
bool RockResponse::serializeToByteArray(ByteArray::ptr bytearray)
{
    try {
        ...
        _LOG_ERROR(g_logger) << "RockResponse serializeToByteArray error";
    }
    return false;
}
...
bool RockResponse::parseFromByteArray(ByteArray::ptr bytearray)
{
    try {
        ...
        _LOG_ERROR(g_logger) << "RockResponse parseFromByteArray error";
    }
    return false;
}
...
bool RockNotify::serializeToByteArray(ByteArray::ptr bytearray)
{
    try {
        ...
        _LOG_ERROR(g_logger) << "RockNotify serializeToByteArray error";
    }
    return false;
}
...
bool RockNotify::parseFromByteArray(ByteArray::ptr bytearray)
{
    try {
        ...
        _LOG_ERROR(g_logger) << "RockNotify parseFromByteArray error";
    }
    return false;
}
...
Message::ptr RockMessageDecoder::parseFrom(Stream::ptr stream)
{
    try {
        ...
        if (rt <= 0) {
            _LOG_DEBUG(g_logger) << "RockMessageDecoder decode head error rt=" << rt << " "
                                 << strerror(errno);
            return nullptr;
        }

        if (memcmp(header.magic, s_rock_magic, sizeof(s_rock_magic))) {
            _LOG_ERROR(g_logger) << "RockMessageDecoder head.magic error";
            return nullptr;
        }

        if (header.version != 0x1) {
            _LOG_ERROR(g_logger) << "RockMessageDecoder head.version != 0x1";
            return nullptr;
        }

        header.length = base::byteswapOnLittleEndian(header.length);
        if ((uint32_t)header.length >= g_rock_protocol_max_length->getValue()) {
            _LOG_ERROR(g_logger) << "RockMessageDecoder head.length(" << header.length
                                 << ") >=" << g_rock_protocol_max_length->getValue();
            return nullptr;
        }
        base::ByteArray::ptr ba = std::make_shared<base::ByteArray>();
        rt = stream->readFixSize(ba, header.length);
        if (rt <= 0) {
            _LOG_ERROR(g_logger) << "RockMessageDecoder read body fail length=" << header.length
                                 << " rt=" << rt << " errno=" << errno << " - " << strerror(errno);
            return nullptr;
        }

        ba->setPosition(0);
        // _LOG_INFO(g_logger) << ba->toHexString();
        if (header.flag & 0x1) { // gizp
            auto zstream = base::ZlibStream::CreateGzip(false);
            if (zstream->write(ba, -1) != Z_OK) {
                _LOG_ERROR(g_logger) << "RockMessageDecoder ungzip error";
                return nullptr;
            }
            if (zstream->flush() != Z_OK) {
                _LOG_ERROR(g_logger) << "RockMessageDecoder ungzip flush error";
                return nullptr;
            }
        ...
        switch (type) {
            ...
            case Message::NOTIFY:
                ...
                break;
            default:
                _LOG_ERROR(g_logger) << "RockMessageDecoder invalid type=" << (int)type;
                return nullptr;
        }

        if (!msg->parseFromByteArray(ba)) {
            _LOG_ERROR(g_logger) << "RockMessageDecoder parseFromByteArray fail type=" << (int)type;
            return nullptr;
        }
        return msg;
    } catch (std::exception &e) {
        _LOG_ERROR(g_logger) << "RockMessageDecoder except:" << e.what();
    } catch (...) {
        _LOG_ERROR(g_logger) << "RockMessageDecoder except";
    }
    return nullptr;
}
...
int32_t RockMessageDecoder::serializeTo(Stream::ptr stream, Message::ptr msg)
{
    ...
    if ((uint32_t)header.length >= g_rock_protocol_gzip_min_length->getValue()) {
        auto zstream = base::ZlibStream::CreateGzip(true);
        if (zstream->write(ba, -1) != Z_OK) {
            _LOG_ERROR(g_logger) << "RockMessageDecoder serializeTo gizp error";
            return -1;
        }
        if (zstream->flush() != Z_OK) {
            _LOG_ERROR(g_logger) << "RockMessageDecoder serializeTo gizp flush error";
            return -2;
        }
    ...
    if (rt <= 0) {
        _LOG_ERROR(g_logger) << "RockMessageDecoder serializeTo write header fail rt=" << rt
                             << " errno=" << errno << " - " << strerror(errno);
        return -3;
    }
    rt = stream->writeFixSize(ba, ba->getReadSize());
    if (rt <= 0) {
        _LOG_ERROR(g_logger) << "RockMessageDecoder serializeTo write body fail rt=" << rt
                             << " errno=" << errno << " - " << strerror(errno);
        return -4;
    }
