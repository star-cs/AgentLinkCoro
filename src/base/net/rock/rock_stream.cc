...
static base::ConfigVar<
    std::unordered_map<std::string, std::unordered_map<std::string, std::string> > >::ptr
    g_rock_services = base::Config::Lookup(
        "rock_services",
        std::unordered_map<std::string, std::unordered_map<std::string, std::string> >(),
        "rock_services");

// static base::ConfigVar<std::unordered_map<std::string
//     ,std::unordered_map<std::string, std::string> > >::ptr g_rock_services =
//     base::Config::Lookup("rock_services", std::unordered_map<std::string
//     ,std::unordered_map<std::string, std::string> >(), "rock_services");

std::string RockResult::toString() const
{
    std::stringstream ss;
    ss << "[RockResult result=" << result << " result_str=" << resultStr << " used=" << used
       << " response=" << (response ? response->toString() : "null")
       << " request=" << (request ? request->toString() : "null") << " server=" << server << "]";
    return ss.str();
}

RockStream::RockStream(Socket::ptr sock)
    : AsyncSocketStream(sock, true), m_decoder(std::make_shared<RockMessageDecoder>())
{
    _LOG_DEBUG(g_logger) << "RockStream::RockStream " << this << " "
                         << (sock ? sock->toString() : "");
}
...
RockResult::ptr RockStream::request(RockRequest::ptr req, uint32_t timeout_ms)
{
    if (isConnected()) {
        if (req->getSn() == 0) {
            req->setSn(base::Atomic::addFetch(m_sn));
        }
        ...
        addCtx(ctx);
        uint64_t ts = base::GetCurrentMS();
        ctx->timer = base::IOManager::GetThis()->addTimer(
        ...
        auto rt = std::make_shared<RockResult>(ctx->result, ctx->resultStr,
                                               base::GetCurrentMS() - ts, ctx->response, req);
        rt->server = getRemoteAddressString();
        return rt;
    } else {
        auto rt = std::make_shared<RockResult>(AsyncSocketStream::NOT_CONNECT,
                                               "not_connect " + getRemoteAddressString(), 0,
                                               nullptr, req);
        rt->server = getRemoteAddressString();
        return rt;
    }
...
{
    //_LOG_INFO(g_logger) << "doRecv " << this;
    ...
    if (type == Message::RESPONSE) {
        ...
        if (!rsp) {
            _LOG_WARN(g_logger) << "RockStream doRecv response not RockResponse: "
                                << msg->toString();
            return nullptr;
        }
        RockCtx::ptr ctx = getAndDelCtxAs<RockCtx>(rsp->getSn());
        if (!ctx) {
            _LOG_WARN(g_logger) << "RockStream request timeout reponse=" << rsp->toString();
            return nullptr;
        }
        ...
        if (!req) {
            _LOG_WARN(g_logger) << "RockStream doRecv request not RockRequest: " << msg->toString();
            return nullptr;
        }
        if (m_requestHandler) {
            ...
            _LOG_WARN(g_logger) << "unhandle request " << req->toString();
        }
    } else if (type == Message::NOTIFY) {
        auto nty = std::dynamic_pointer_cast<RockNotify>(msg);
        if (!nty) {
            _LOG_WARN(g_logger) << "RockStream doRecv notify not RockNotify: " << msg->toString();
            return nullptr;
        }
        ...
        if (m_notifyHandler) {
            ...
            _LOG_WARN(g_logger) << "unhandle notify " << nty->toString();
        }
    } else {
        _LOG_WARN(g_logger) << "RockStream recv unknow type=" << type
                            << " msg: " << msg->toString();
    }
    return nullptr;
}

void RockStream::handleRequest(base::RockRequest::ptr req)
{
    base::RockResponse::ptr rsp = req->createResponse();
...
void RockStream::handleNotify(base::RockNotify::ptr nty)
{
...
bool RockConnection::connect(base::Address::ptr addr)
{
    m_socket = base::Socket::CreateTCP(addr);
...
static SocketStream::ptr create_rock_stream(const std::string &domain, const std::string &service,
                                            ServiceItemInfo::ptr info)
{
    //_LOG_INFO(g_logger) << "create_rock_stream: " << info->toString();
    base::IPAddress::ptr addr = base::Address::LookupAnyIPAddress(info->getIp());
    if (!addr) {
        _LOG_ERROR(g_logger) << "invalid service info: " << info->toString();
        return nullptr;
    }
    ...
    base::WorkerMgr::GetInstance()->schedule("service_io", [conn, addr, domain, service]() {
        if (domain == "logserver") {
            conn->setConnectCb([conn](base::AsyncSocketStream::ptr) {
                base::RockRequest::ptr req = std::make_shared<base::RockRequest>();
                req->setCmd(100);
                logserver::LoginRequest login_req;
                login_req.set_ipport(base::Module::GetServiceIPPort("rock"));
                login_req.set_host(base::GetHostName());
                login_req.set_pid(getpid());
                req->setAsPB(login_req);
                auto r = conn->request(req, 200);
                if (r->result) {
                    _LOG_ERROR(g_logger) << "logserver login error: " << r->toString();
                    return false;
                }
                return true;
            });
                                           ...
                                           RockRequest::ptr req, uint32_t timeout_ms, uint64_t idx)
{
    ...
    if (!lb) {
        return std::make_shared<RockResult>(ILoadBalance::NO_SERVICE, "no_service", 0, nullptr,
                                            req);
    }
    auto conn = lb->get(idx);
    if (!conn) {
        return std::make_shared<RockResult>(ILoadBalance::NO_CONNECTION, "no_connection", 0,
                                            nullptr, req);
    }
    uint64_t ts = base::GetCurrentMS();
    ...
    uint64_t ts2 = base::GetCurrentMS();
    if (r->result == 0) {
        stats.incOks(1);
