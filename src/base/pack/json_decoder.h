...
namespace pack
{
    ...
    {
    public:
        JsonDecoder(const Json::Value &value) : m_value(value), m_cur(&m_value) {}

        // template<class T>
        // _NOT_PACK(T, bool) decode(const std::string& name, T& v, const PackFlag& flag) {
...
#undef XX_DECODE
    ...
    {
        Json::Value jvalue;
        if (!base::JsonUtil::FromString(jvalue, json)) {
            // TODO
            return false;
        }
