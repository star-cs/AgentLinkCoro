...
static base::ConfigVar<std::string>::ptr g_module_path =
...
bool Module::handleRequest(base::Message::ptr req, base::Message::ptr rsp, base::Stream::ptr stream)
{
    _LOG_DEBUG(g_logger) << "handleRequest req=" << req->toString() << " rsp=" << rsp->toString()
                         << " stream=" << stream;
    return true;
}

bool Module::handleNotify(base::Message::ptr notify, base::Stream::ptr stream)
{
    _LOG_DEBUG(g_logger) << "handleNotify nty=" << notify->toString() << " stream=" << stream;
    return true;
}
...
bool Module::onConnect(base::Stream::ptr stream)
{
    return true;
}

bool Module::onDisconnect(base::Stream::ptr stream)
{
    return true;
}
    ...
    for (auto &i : svrs) {
        ...
        for (auto &s : socks) {
            ...
            if (str.find("0.0.0.0") == 0) {
                ip_and_port = base::GetIPv4() + ":" + std::to_string(addr->getPort());
            } else {
...
bool RockModule::handleRequest(base::Message::ptr req, base::Message::ptr rsp,
                               base::Stream::ptr stream)
{
    auto rock_req = std::dynamic_pointer_cast<base::RockRequest>(req);
    auto rock_rsp = std::dynamic_pointer_cast<base::RockResponse>(rsp);
    auto rock_stream = std::dynamic_pointer_cast<base::RockStream>(stream);
    return handleRockRequest(rock_req, rock_rsp, rock_stream);
}

bool RockModule::handleNotify(base::Message::ptr notify, base::Stream::ptr stream)
{
    auto rock_nty = std::dynamic_pointer_cast<base::RockNotify>(notify);
    auto rock_stream = std::dynamic_pointer_cast<base::RockStream>(stream);
