...
{
    base::LogFormatter::ptr new_val = std::make_shared<base::LogFormatter>(val);
    if (new_val->isError()) {
...
base::ConfigVar<std::set<LogDefine> >::ptr g_log_defines =
    base::Config::Lookup("logs", std::set<LogDefine>(), "logs config");
...
struct LogIniter {
    LogIniter()
    {
        g_log_defines->addListener([](const std::set<LogDefine> &old_value,
            ...
            _LOG_INFO(_LOG_ROOT()) << "on_logger_conf_changed";
            for (auto &i : new_value) {
                auto it = old_value.find(i);
                base::Logger::ptr logger;
                if (it == old_value.end()) {
                    // 新增logger
