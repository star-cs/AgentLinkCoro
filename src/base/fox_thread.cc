...
void FoxThread::read_cb(evutil_socket_t sock, short which, void *args)
{
    ...
    if (recv(sock, cmd, sizeof(cmd), 0) > 0) {
        ...
        for (auto it = callbacks.begin(); it != callbacks.end(); ++it) {
            if (*it) {
                // _ASSERT(thread == GetThis());
                try {
                    (*it)();
                } catch (std::exception &ex) {
                    _LOG_ERROR(g_logger) << "exception:" << ex.what();
                } catch (const char *c) {
                    _LOG_ERROR(g_logger) << "exception:" << c;
                } catch (...) {
                    _LOG_ERROR(g_logger) << "uncatch exception";
                }
            } else {
...
FoxThread::FoxThread(const std::string &name, struct event_base *base)
    : m_read(0), m_write(0), m_base(NULL), m_event(NULL), m_thread(NULL), m_name(name),
      m_working(false), m_start(false), m_total(0)
{
    int fds[2];
    if (evutil_socketpair(AF_UNIX, SOCK_STREAM, 0, fds) == -1) {
        // _LOG_ERROR(g_logger) << "FoxThread init error";
...
{
    if (m_thread) {
        // _LOG_ERROR(g_logger) << "FoxThread is running";
...
{
    do {
        base::Atomic::addFetch(m_total, (uint64_t)1);
        RWMutex::WriteLock lock(m_mutex);
        if (!m_advance) {
...
void FoxThreadManager::init()
{
    ...
    for (auto i : m) {
        auto num = base::GetParamValue(i.second, "num", 0);
        auto name = i.first;
...
void FoxThreadManager::start()
{
    for (auto i : m_threads) {
        _LOG_INFO(g_logger) << "thread: " << i.first << " start begin";
        i.second->start();
        _LOG_INFO(g_logger) << "thread: " << i.first << " start end";
    }
}
...
{
    for (auto i : m_threads) {
        _LOG_INFO(g_logger) << "thread: " << i.first << " stop begin";
        i.second->stop();
        _LOG_INFO(g_logger) << "thread: " << i.first << " stop end";
    }
    for (auto i : m_threads) {
        _LOG_INFO(g_logger) << "thread: " << i.first << " join begin";
        i.second->join();
        _LOG_INFO(g_logger) << "thread: " << i.first << " join end";
    }
}
