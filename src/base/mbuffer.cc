...
int MBuffer::var_encode(uint8_t *dst, size_t dst_len, size_t &len, uint64_t src)
{ // 把uint64_t src拷贝到dst中 传出实际长度
    uint8_t flag = 0;
...
int MBuffer::var_decode(uint64_t &dst, size_t &len, const uint8_t *src, size_t src_len)
{ // 把const uint8_t* src分解成uint64_t 传出实际长度
    if (src_len < 1) {
        return -1;
    }
    len = 1 << (src[0] >> 6);
...
MBuffer::SegmentData::SegmentData() : m_start(nullptr), m_length(0)
{
}

MBuffer::SegmentData::SegmentData(size_t length) : m_length(length)
{
    m_array.reset(new uint8_t[length], [](uint8_t *ptr) { delete[] ptr; });
...
MBuffer::SegmentData MBuffer::SegmentData::slice(size_t start, size_t length)
{
    ...
    _ASSERT(m_length >= start);
    _ASSERT(m_length >= length + start);
...
MBuffer::Segment::Segment(size_t length)
    : // 一个segment里只有一个segmentData
      m_writeIndex(0), m_data(length)
{
    _ASSERT(m_writeIndex <= m_data.m_length);
}

MBuffer::Segment::Segment(SegmentData seg_data)
    : m_writeIndex(seg_data.getLength()), m_data(seg_data)
{
    _ASSERT(m_writeIndex <= m_data.m_length);
}

MBuffer::Segment::Segment(void *buffer, size_t length) : m_writeIndex(0), m_data(buffer, length)
{
    _ASSERT(m_writeIndex <= m_data.m_length);
}
...
{
    _ASSERT(length <= readAvailable());
    _ASSERT(m_writeIndex == readAvailable());
    m_writeIndex = length;
...
MBuffer::MBuffer(MBuffer &&copy) noexcept
{
    // 直接移动资源，避免拷贝
    m_segments = std::move(copy.m_segments);
    m_readAvailable = copy.m_readAvailable;
    m_writeAvailable = copy.m_writeAvailable;
    m_endian = copy.m_endian;
    
    // 更新迭代器位置
    if (copy.m_writeIt == copy.m_segments.end()) {
        m_writeIt = m_segments.end();
    } else {
        // 计算迭代器位置偏移
        auto offset = std::distance(copy.m_segments.begin(), copy.m_writeIt);
        m_writeIt = m_segments.begin();
        std::advance(m_writeIt, offset);
    }
    
    // 重置源对象状态
    copy.m_readAvailable = 0;
    copy.m_writeAvailable = 0;
    copy.m_writeIt = copy.m_segments.end();
    copy.m_endian = 1;
}
...
MBuffer &MBuffer::operator=(MBuffer &&copy) noexcept
{
    if (this != &copy) {
        // 释放当前资源
        m_segments.clear();
...
const iovec MBuffer::readBuffer(size_t length, bool reallocate) const
{
    ...
    if (m_writeIt != m_segments.end()
        && m_writeIt->writeAvailable()
               >= readAvailable()) { // 当前块的可写空间远大于所有块的可读空间
        copyOut(m_writeIt->writeBuffer().getStart(), readAvailable());
        Segment newSegment =
            Segment(m_writeIt->writeBuffer().slice(0, readAvailable())); // 形成新的一块
...
iovec MBuffer::writeBuffer(size_t length, bool reallocate)
{
    ...
    if (writeAvailable() == 0) {
        reserve(length);
        _ASSERT(m_writeIt != m_segments.end());
        _ASSERT(m_writeIt->writeAvailable() >= length);
...
void MBuffer::copyIn(const MBuffer &buffer, size_t length, size_t pos)
{
    if (pos > buffer.readAvailable()) {
        _ASSERT("positioin out of range");
        return;
    }
    if (length == (size_t)~0) {
        length = buffer.readAvailable() - pos;
    }
    _ASSERT(buffer.readAvailable() >= length + pos);
    invariant();
    if (length == 0)
        return;
    if (m_writeIt != m_segments.end() && m_writeIt->readAvailable() != 0) {
    ...
    _ASSERT(it != buffer.m_segments.end());
    for (; it != buffer.m_segments.end(); ++it) {
        ...
        if (m_readAvailable != 0 && it == buffer.m_segments.begin()) {
            ...
            --previousIt;
            if ((char *)previousIt->readBuffer().getStart() + previousIt->readBuffer().getLength()
                    == (char *)it->readBuffer().getStart() + pos
                && previousIt->m_data.m_array.get() == it->m_data.m_array.get()) {
                _ASSERT(previousIt->writeAvailable() == 0);
...
void MBuffer::copyOut(void *buffer, size_t length, size_t pos) const
{
    ...
    _ASSERT(length + pos <= readAvailable());
...
std::string MBuffer::getDelimited(char delimiter, bool eofIsDelimiter, bool includeDelimiter)
{
    /*
    ptrdiff_t offset = find(delimiter, ~0);
    _ASSERT(offset >= -1);
    if (offset == -1 && !eofIsDelimiter) {
        _ASEERT("getDelimited failed");
    }
...
void MBuffer::visit(std::function<void(const void *, size_t)> dg, size_t length) const
{
    if (length == (size_t)~0) {
        length = readAvailable();
    }
...
int MBuffer::opCmp(const MBuffer &rhs) const
{
    ...
    while (leftIt != m_segments.end() && rightIt != rhs.m_segments.end()) {
        _ASSERT(leftOffset <= leftIt->readAvailable());
        _ASSERT(rightOffset <= rightIt->readAvailable());

        size_t tocompare =
            std::min(leftIt->readAvailable() - leftOffset, rightIt->readAvailable() - rightOffset);
        if (tocompare == 0) {
            break;
        }
        int result = memcmp((const unsigned char *)leftIt->readBuffer().getStart() + leftOffset,
        ...
        for (it = m_segments.begin(); it != m_segments.end(); it++) {
            ...
            if (nextIt != m_segments.end()) {
                ...
                if (segment.writeAvailable() == 0 &&  // 当前块是满的 且下一块有数据
                    ...
                    _ASSERT((const unsigned char*)segment.readBuffer().start() +
                    ...
                    next.readAvailable() == 0) {
                    _ASSERT((const unsigned char*)segment.writeBuffer().start() +
                        segment.writeAvailable() != next.writeBuffer().start() ||
                        segement.m_data.m_array.get() != next.m_data.m_array.get());
                }
            }
        }
        _ASSERT(read == m_readAvailable);
        _ASSERT(write == m_writeAvailable);
        _ASSERT(write != 0 || (write == 0 && m_writeIt == m_segments.end()));
#endif
}
} // namespace base