...
#include "base/pack/yaml_decoder.h"
#include "base/pack/yaml_encoder.h"
...
public:
    ...
    {
        T t;
        base::pack::DecodeFromYamlString(str, t, 0);
        return t;
    }
};
...
{
public:
    std::string operator()(const T &v) { return base::pack::EncodeToYamlString(v, 0); }
};

#define _DEFINE_CONFIG(type, name, attr, def, desc)                                                \
    static base::ConfigVar<type, base::PackDecodeCast<type>, base::PackEncodeCast<type>>::ptr      \
        name = base::Config::Lookup<type, base::PackDecodeCast<type>, base::PackEncodeCast<type>>( \
            attr, (type)def, desc);

#define _GET_CONFIG(type, attr)                                                                    \
    base::Config::Lookup<type, base::PackDecodeCast<type>, base::PackEncodeCast<type>>(attr)
...
public:
    ...
    std::string toString() override
    {
        try {
            ...
            _LOG_ERROR(_LOG_ROOT()) << "ConfigVar::toString exception " << e.what()
...
public:
    ...
    Lookup(const std::string &name, const T &default_value, const std::string &description = "")
    {
        ...
        if (name.find_first_not_of("abcdefghikjlmnopqrstuvwxyz._012345678") != std::string::npos) {
            _LOG_ERROR(_LOG_ROOT()) << "Lookup name invalid " << name;
