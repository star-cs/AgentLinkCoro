...
public:
    ...
    FoxRedis(base::FoxThread *thr, const std::map<std::string, std::string> &conf);
    ~FoxRedis();
...
public:
    ...
    FoxRedisCluster(base::FoxThread *thr, const std::map<std::string, std::string> &conf);
    ~FoxRedisCluster();
...
typedef base::Singleton<RedisManager> RedisMgr;
