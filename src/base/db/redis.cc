...
static base::ConfigVar<std::map<std::string, std::map<std::string, std::string> > >::ptr g_redis =
...
Redis::Redis(const std::map<std::string, std::string> &conf)
{
    ...
    m_port = base::TypeUtil::Atoi(tmp.substr(pos + 1));
    m_passwd = get_value(conf, "passwd");
    m_logEnable = base::TypeUtil::Atoi(get_value(conf, "log_enable", "1"));

    tmp = get_value(conf, "timeout_com");
    if (tmp.empty()) {
        tmp = get_value(conf, "timeout");
    }
    uint64_t v = base::TypeUtil::Atoi(tmp);
    ...
    if (c) {
        ...
        if (!m_passwd.empty()) {
            ...
            if (r->type != REDIS_REPLY_STATUS) {
                _LOG_ERROR(g_logger) << "auth reply type error:" << r->type << "(" << m_host << ":"
                                     << m_port << ", " << m_name << ")";
                return false;
            }
            if (!r->str) {
                _LOG_ERROR(g_logger) << "auth reply str error: NULL(" << m_host << ":" << m_port
                                     << ", " << m_name << ")";
                return false;
            }
            if (strcmp(r->str, "OK") == 0) {
                ...
                _LOG_ERROR(g_logger) << "auth error: " << r->str << "(" << m_host << ":" << m_port
                                     << ", " << m_name << ")";
                return false;
            }
        }
...
ReplyPtr Redis::cmd(const char *fmt, va_list ap)
{
    ...
    if (!r) {
        if (m_logEnable) {
            _LOG_ERROR(g_logger) << "redisCommand error: (" << fmt << ")(" << m_host << ":"
                                 << m_port << ")(" << m_name << ")";
        }
        return nullptr;
    }
    ...
    if (m_logEnable) {
        _LOG_ERROR(g_logger) << "redisCommand error: (" << fmt << ")(" << m_host << ":" << m_port
                             << ")(" << m_name << ")"
                             << ": " << r->str;
    }
    return nullptr;
...
ReplyPtr Redis::cmd(const std::vector<std::string> &argv)
{
    ...
    if (!r) {
        if (m_logEnable) {
            _LOG_ERROR(g_logger) << "redisCommandArgv error: (" << m_host << ":" << m_port << ")("
                                 << m_name << ")";
        }
        return nullptr;
    }
...
ReplyPtr Redis::getReply()
{
    ...
    if (m_logEnable) {
        _LOG_ERROR(g_logger) << "redisGetReply error: (" << m_host << ":" << m_port << ")("
                             << m_name << ")";
    }
    return nullptr;
}
...
FoxRedis::FoxRedis(base::FoxThread *thr, const std::map<std::string, std::string> &conf)
...
void FoxRedis::OnAuthCb(redisAsyncContext *c, void *rp, void *priv)
{
    ...
    if (!r) {
        _LOG_ERROR(g_logger) << "auth error:(" << fr->m_host << ":" << fr->m_port << ", "
                             << fr->m_name << ")";
        return;
    }
    if (r->type != REDIS_REPLY_STATUS) {
        _LOG_ERROR(g_logger) << "auth reply type error:" << r->type << "(" << fr->m_host << ":"
                             << fr->m_port << ", " << fr->m_name << ")";
        return;
    }
    if (!r->str) {
        _LOG_ERROR(g_logger) << "auth reply str error: NULL(" << fr->m_host << ":" << fr->m_port
                             << ", " << fr->m_name << ")";
        return;
    }
    if (strcmp(r->str, "OK") == 0) {
        _LOG_INFO(g_logger) << "auth ok: " << r->str << "(" << fr->m_host << ":" << fr->m_port
                            << ", " << fr->m_name << ")";
    } else {
        _LOG_ERROR(g_logger) << "auth error: " << r->str << "(" << fr->m_host << ":" << fr->m_port
                             << ", " << fr->m_name << ")";
    }
}
...
void FoxRedis::ConnectCb(const redisAsyncContext *c, int status)
{
    ...
    if (!status) {
        _LOG_INFO(g_logger) << "FoxRedis::ConnectCb " << c->c.tcp.host << ":" << c->c.tcp.port
                            << " success";
        ...
        if (!ar->m_passwd.empty()) {
            int rt = redisAsyncCommand(ar->m_context.get(), FoxRedis::OnAuthCb, ar, "auth %s",
                                       ar->m_passwd.c_str());
            if (rt) {
                _LOG_ERROR(g_logger) << "FoxRedis Auth fail: " << rt;
            }
        }

    } else {
        _LOG_ERROR(g_logger) << "FoxRedis::ConnectCb " << c->c.tcp.host << ":" << c->c.tcp.port
                             << " fail, error:" << c->errstr;
...
{
    _LOG_INFO(g_logger) << "FoxRedis::DisconnectCb " << c->c.tcp.host << ":" << c->c.tcp.port
                        << " status:" << status;
    FoxRedis *ar = static_cast<FoxRedis *>(c->data);
    ar->m_status = UNCONNECTED;
}
...
void FoxRedis::CmdCb(redisAsyncContext *ac, void *r, void *privdata)
{
    ...
    if (ctx->timeout) {
        delete ctx;
        // if(ctx && ctx->fiber) {
        //     _LOG_ERROR(g_logger) << "redis cmd: '" << ctx->cmd << "' timeout("
    ...
    if (ac->err) {
        if (m_logEnable) {
            base::replace(ctx->cmd, "\r\n", "\\r\\n");
            _LOG_ERROR(g_logger) << "redis cmd: '" << ctx->cmd << "' "
                                 << "(" << ac->err << ") " << ac->errstr;
        }
        if (ctx->fctx->fiber) {
...
void FoxRedis::TimeCb(int fd, short event, void *d)
{
    ...
    _ASSERT(ar->m_thread == base::FoxThread::GetThis());
...
//     _LOG_INFO(g_logger) << "DelayTimeCb";
...
bool FoxRedis::init()
{
    if (m_thread == base::FoxThread::GetThis()) {
        return pinit();
    } else {
...
bool FoxRedis::pinit()
{
    // _LOG_INFO(g_logger) << "pinit m_status=" << m_status;
    if (m_status != UNCONNECTED) {
        return true;
    }
    auto ctx = redisAsyncConnect(m_host.c_str(), m_port);
    if (!ctx) {
        _LOG_ERROR(g_logger) << "redisAsyncConnect (" << m_host << ":" << m_port << ") null";
        return false;
    }
    if (ctx->err) {
        _LOG_ERROR(g_logger) << "Error:(" << ctx->err << ")" << ctx->errstr;
        return false;
    }
    ctx->data = this;
    ...
    m_context.reset(ctx, base::nop<redisAsyncContext>);
...
ReplyPtr FoxRedis::cmd(const char *fmt, va_list ap)
{
    ...
    if (len == -1) {
        _LOG_ERROR(g_logger) << "redis fmt error: " << fmt;
        return nullptr;
    }
    ...
    //     free(buf);
    // }
    // ctx->scheduler = base::Scheduler::GetThis();
    // ctx->fiber = base::Fiber::GetThis();
    // ctx->thread = m_thread;

    FCtx fctx;
    fctx.cmd.append(buf, len);
    free(buf);
    fctx.scheduler = base::Scheduler::GetThis();
    fctx.fiber = base::Fiber::GetThis();

    m_thread->dispatch(std::bind(&FoxRedis::pcmd, this, &fctx));
    base::Fiber::YieldToHold();
    return fctx.rpy;
}
...
ReplyPtr FoxRedis::cmd(const std::vector<std::string> &argv)
{
    ...
    do {
        ...
        if (len == -1 || !buf) {
            _LOG_ERROR(g_logger) << "redis fmt error";
            return nullptr;
        }
        fctx.cmd.append(buf, len);
        free(buf);
...
{
    if (m_status == UNCONNECTED) {
        _LOG_INFO(g_logger) << "redis (" << m_host << ":" << m_port << ") unconnected "
                            << fctx->cmd;
        init();
        if (fctx->fiber) {
...
FoxRedis::Ctx::Ctx(FoxRedis *r)
    : ev(nullptr), timeout(false), rds(r)
      //,scheduler(nullptr)
      ,
      thread(nullptr)
{
    base::Atomic::addFetch(rds->m_ctxCount, 1);
}

FoxRedis::Ctx::~Ctx()
{
    // cancelEvent();
    _ASSERT(thread == base::FoxThread::GetThis());
    // _ASSERT(destory == 0);
    base::Atomic::subFetch(rds->m_ctxCount, 1);
    //++destory;
    // cancelEvent();
    if (ev) {
        evtimer_del(ev);
...
{
    // if(ev) {
    //     if(thread == base::FoxThread::GetThis()) {
    //         evtimer_del(ev);
    //         event_free(ev);
    //     } else {
    //         auto e = ev;
...
void FoxRedis::Ctx::EventCb(int fd, short event, void *d)
{
    ...
    if (ctx->rds->m_logEnable) {
        base::replace(ctx->cmd, "\r\n", "\\r\\n");
        _LOG_INFO(g_logger) << "redis cmd: '" << ctx->cmd << "' reach timeout "
                            << (ctx->rds->m_cmdTimeout.tv_sec * 1000
                                + ctx->rds->m_cmdTimeout.tv_usec / 1000)
                            << "ms";
    }
...
FoxRedisCluster::FoxRedisCluster(base::FoxThread *thr,
...
void FoxRedisCluster::OnAuthCb(redisClusterAsyncContext *c, void *rp, void *priv)
{
    ...
    redisReply *r = (redisReply *)rp;
    if (!r) {
        _LOG_ERROR(g_logger) << "auth error:(" << fr->m_host << ", " << fr->m_name << ")";
        return;
    }
    if (r->type != REDIS_REPLY_STATUS) {
        _LOG_ERROR(g_logger) << "auth reply type error:" << r->type << "(" << fr->m_host << ", "
    ...
    if (!r->str) {
        _LOG_ERROR(g_logger) << "auth reply str error: NULL(" << fr->m_host << ", " << fr->m_name
                             << ")";
        return;
    }
    if (strcmp(r->str, "OK") == 0) {
        _LOG_INFO(g_logger) << "auth ok: " << r->str << "(" << fr->m_host << ", " << fr->m_name
                            << ")";
    } else {
        _LOG_ERROR(g_logger) << "auth error: " << r->str << "(" << fr->m_host << ", " << fr->m_name
                             << ")";
    }
}
...
void FoxRedisCluster::ConnectCb(const redisAsyncContext *c, int status)
{
    ...
    if (!status) {
        _LOG_INFO(g_logger) << "FoxRedisCluster::ConnectCb " << c->c.tcp.host << ":"
                                 << c->c.tcp.port << " success";
        if (!ar->m_passwd.empty()) {
            int rt = redisClusterAsyncCommand(ar->m_context.get(), FoxRedisCluster::OnAuthCb, ar,
                                              "auth %s", ar->m_passwd.c_str());
            if (rt) {
                _LOG_ERROR(g_logger) << "FoxRedisCluster Auth fail: " << rt;
            }
        }
    } else {
        _LOG_ERROR(g_logger) << "FoxRedisCluster::ConnectCb " << c->c.tcp.host << ":"
...
{
    _LOG_INFO(g_logger) << "FoxRedisCluster::DisconnectCb " << c->c.tcp.host << ":" << c->c.tcp.port
                        << " status:" << status;
}
...
bool FoxRedisCluster::pinit()
{
    ...
    _LOG_INFO(g_logger) << "FoxRedisCluster pinit:" << m_host;
    ...
    if (!ctx) {
        _LOG_ERROR(g_logger) << "redisClusterAsyncConnect (" << m_host << ") null";
        return false;
    }
    if (ctx->err) {
        _LOG_ERROR(g_logger) << "Error:(" << ctx->err << ")" << ctx->errstr
    ...
    m_status = CONNECTED;
    // m_context.reset(ctx, redisAsyncFree);
    m_context.reset(ctx, base::nop<redisClusterAsyncContext>);
...
void FoxRedisCluster::pcmd(FCtx *fctx)
{
    if (m_status != CONNECTED) {
        _LOG_INFO(g_logger) << "redis (" << m_host << ") unconnected " << fctx->cmd;
        init();
        if (fctx->fiber) {
            fctx->scheduler->schedule(&fctx->fiber);
        }
...
{
    // _LOG_INFO(g_logger) << "cancelEvent " << base::FoxThread::GetThis()
    //            << " - " << thread
    ...
    //    //_LOG_INFO(g_logger) << "cancelEvent thread=" << thread << " " << thread->getId()
    //    //           << " this=" << base::FoxThread::GetThis();
    //    //_ASSERT(thread == base::FoxThread::GetThis());
    //}
    // _ASSERT(!base::IOManager::GetThis());
    ////if(base::Atomic::addFetch(cancel_count) > 1) {
    ////    return;
    ////}
    ////_ASSERT(!base::Fiber::GetThis());
    ////base::RWMutex::WriteLock lock(mutex);
    // if(++cancel_count > 1) {
    //     return;
    // }
    // if(ev) {
...
IRedis::ptr RedisManager::get(const std::string &name)
{
    ...
    if (r->getType() == IRedis::FOX_REDIS || r->getType() == IRedis::FOX_REDIS_CLUSTER) {
        ...
        return std::shared_ptr<IRedis>(r, base::nop<IRedis>);
    }
    lock.unlock();
...
void RedisManager::init()
{
    ...
    for (auto &i : m_config) {
        ...
        auto pool = base::TypeUtil::Atoi(get_value(i.second, "pool"));
        auto passwd = get_value(i.second, "passwd");
        total += pool;
        for (int n = 0; n < pool; ++n) {
            if (type == "redis") {
                base::Redis *rds(new base::Redis(i.second));
                rds->connect();
                rds->setLastActiveTime(time(0));
                base::RWMutex::WriteLock lock(m_mutex);
                m_datas[i.first].push_back(rds);
                base::Atomic::addFetch(done, 1);
            } else if (type == "redis_cluster") {
                base::RedisCluster *rds(new base::RedisCluster(i.second));
                rds->connect();
                rds->setLastActiveTime(time(0));
                base::RWMutex::WriteLock lock(m_mutex);
                m_datas[i.first].push_back(rds);
                ...
                auto conf = i.second;
                auto name = i.first;
                base::FoxThreadMgr::GetInstance()->dispatch("redis", [this, conf, name, &done]() {
                    base::FoxRedis *rds(new base::FoxRedis(base::FoxThread::GetThis(), conf));
                    rds->init();
                    rds->setName(name);
                ...
                auto name = i.first;
                base::FoxThreadMgr::GetInstance()->dispatch("redis", [this, conf, name, &done]() {
                    base::FoxRedisCluster *rds(
                        new base::FoxRedisCluster(base::FoxThread::GetThis(), conf));
                    rds->init();
                    rds->setName(name);
