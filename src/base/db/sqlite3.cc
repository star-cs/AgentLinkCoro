...
static base::ConfigVar<std::map<std::string, std::map<std::string, std::string> > >::ptr
    g_sqlite3_dbs = base::Config::Lookup(
...
{
    return base::protected_make_shared<SQLite3>(db);
}
...
{
    SQLite3Stmt::ptr rt = base::protected_make_shared<SQLite3Stmt>(db);
...
{
    return bind(idx, base::Time2Str(value));
}
...
time_t SQLite3Data::getTime(int idx)
{
    ...
    return base::Str2Time(str.c_str());
}
...
SQLite3Transaction::~SQLite3Transaction()
{
    if (m_status == 1) {
        ...
        if (m_status == 1) {
            _LOG_ERROR(g_logger) << m_db << " auto_commit=" << m_autoCommit << " fail";
        }
    }
}
...
SQLite3::ptr SQLite3Manager::get(const std::string &name)
{
    ...
    lock.unlock();
    std::string path = base::GetParamValue<std::string>(args, "path");
    if (path.empty()) {
        _LOG_ERROR(g_logger) << "open db name=" << name << " path is null";
        return nullptr;
    }

    if (path.find(":") == std::string::npos) {
        path = base::EnvMgr::GetInstance()->getAbsoluteWorkPath(path);
    }

    sqlite3 *db;
    if (sqlite3_open_v2(path.c_str(), &db, SQLite3::CREATE | SQLite3::READWRITE, nullptr)) {
        _LOG_ERROR(g_logger) << "open db name=" << name << " path=" << path << " fail";
        return nullptr;
    }

    SQLite3 *rt = new SQLite3(db);
    std::string sql = base::GetParamValue<std::string>(args, "sql");
    if (!sql.empty()) {
        if (rt->execute(sql)) {
            _LOG_ERROR(g_logger) << "execute sql=" << sql << " errno=" << rt->getErrno()
                                 << " errstr=" << rt->getErrStr();
            delete rt;
            return nullptr;
        }
...
int SQLite3Manager::execute(const std::string &name, const char *format, va_list ap)
{
    auto conn = get(name);
    if (!conn) {
        _LOG_ERROR(g_logger) << "SQLite3Manager::execute, get(" << name
                             << ") fail, format=" << format;
        return -1;
    }
...
int SQLite3Manager::execute(const std::string &name, const std::string &sql)
{
    auto conn = get(name);
    if (!conn) {
        _LOG_ERROR(g_logger) << "SQLite3Manager::execute, get(" << name << ") fail, sql=" << sql;
        return -1;
    }
...
ISQLData::ptr SQLite3Manager::query(const std::string &name, const char *format, va_list ap)
{
    auto conn = get(name);
    if (!conn) {
        _LOG_ERROR(g_logger) << "SQLite3Manager::query, get(" << name
                             << ") fail, format=" << format;
        return nullptr;
    }
...
ISQLData::ptr SQLite3Manager::query(const std::string &name, const std::string &sql)
{
    auto conn = get(name);
    if (!conn) {
        _LOG_ERROR(g_logger) << "SQLite3Manager::query, get(" << name << ") fail, sql=" << sql;
        return nullptr;
    }
...
SQLite3Transaction::ptr SQLite3Manager::openTransaction(const std::string &name, bool auto_commit)
{
    auto conn = get(name);
    if (!conn) {
        _LOG_ERROR(g_logger) << "SQLite3Manager::openTransaction, get(" << name << ") fail";
        return nullptr;
    }
