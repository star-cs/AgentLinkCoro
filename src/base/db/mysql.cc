...
static base::ConfigVar<std::map<std::string, std::map<std::string, std::string> > >::ptr
    g_mysql_dbs = base::Config::Lookup(
...
static MYSQL *mysql_init(std::map<std::string, std::string> &params, const int &timeout)
{
    ...
    if (mysql == nullptr) {
        _LOG_ERROR(g_logger) << "mysql_init error";
        return nullptr;
    }
    ...
    int port = base::GetParamValue(params, "port", 0);
    std::string host = base::GetParamValue<std::string>(params, "host");
    std::string user = base::GetParamValue<std::string>(params, "user");
    std::string passwd = base::GetParamValue<std::string>(params, "passwd");
    std::string dbname = base::GetParamValue<std::string>(params, "dbname");

    if (mysql_real_connect(mysql, host.c_str(), user.c_str(), passwd.c_str(), dbname.c_str(), port,
                           NULL, 0)
        == nullptr) {
        _LOG_ERROR(g_logger) << "mysql_real_connect(" << host << ", " << port << ", " << dbname
...
bool MySQL::connect()
{
    ...
    m_hasError = false;
    m_poolSize = base::GetParamValue(m_params, "pool", 5);
    m_mysql.reset(m, mysql_close);
    return true;
}

base::IStmt::ptr MySQL::prepare(const std::string &sql)
{
...
int MySQL::execute(const char *format, va_list ap)
{
    ...
    if (r) {
        _LOG_ERROR(g_logger) << "cmd=" << cmd() << ", error: " << getErrStr();
        m_hasError = true;
    } else {
        m_hasError = false;
    }
...
{
    return MySQL::ptr(this, base::nop<MySQL>);
}
...
{
    if (mysql == nullptr) {
        _LOG_ERROR(g_logger) << "mysql_query mysql is null";
        return nullptr;
    }

    if (sql == nullptr) {
        _LOG_ERROR(g_logger) << "mysql_query sql is null";
        return nullptr;
    }

    if (::mysql_query(mysql, sql)) {
        _LOG_ERROR(g_logger) << "mysql_query(" << sql << ") error:" << mysql_error(mysql);
        return nullptr;
    }

    MYSQL_RES *res = mysql_store_result(mysql);
    if (res == nullptr) {
        _LOG_ERROR(g_logger) << "mysql_store_result() error:" << mysql_error(mysql);
    }
    return res;
}
...
MySQLStmt::ptr MySQLStmt::Create(MySQL::ptr db, const std::string &stmt)
{
    ...
    if (mysql_stmt_prepare(st, stmt.c_str(), stmt.size())) {
        _LOG_ERROR(g_logger) << "stmt=" << stmt << " errno=" << mysql_stmt_errno(st)
                             << " errstr=" << mysql_stmt_error(st);
        mysql_stmt_close(st);
        return nullptr;
    }
    int count = mysql_stmt_param_count(st);
    MySQLStmt::ptr rt = base::protected_make_shared<MySQLStmt>(db, st);
...
int MySQLStmt::bindTime(int idx, const time_t &value)
{
    ...
    // return 0;
    return bindString(idx, base::Time2Str(value));
}
...
{
    return base::TypeUtil::Atoi(m_cur[idx]);
}
...
{
    return base::TypeUtil::Atof(m_cur[idx]);
}
...
time_t MySQLRes::getTime(int idx)
{
    ...
    return base::Str2Time(m_cur[idx]);
}
...
MySQLStmtRes::ptr MySQLStmtRes::Create(std::shared_ptr<MySQLStmt> stmt)
{
    ...
    MySQLStmtRes::ptr rt = base::protected_make_shared<MySQLStmtRes>(stmt, eno, errstr);
    if (eno) {
        return rt;
    }
    MYSQL_RES *res = mysql_stmt_result_metadata(stmt->getRaw());
    if (!res) {
        return base::protected_make_shared<MySQLStmtRes>(stmt, stmt->getErrno(), stmt->getErrStr());
    }
...
ISQLData::ptr MySQL::query(const char *format, va_list ap)
{
    ...
    if (!res) {
        m_hasError = true;
        // _LOG_ERROR(g_logger) << "query: " << m_cmd << " fail, errno="
        //     << getErrno() << " errstr=" << getErrStr();
        return nullptr;
    }
    m_hasError = false;
    ISQLData::ptr rt =
        std::make_shared<MySQLRes>(res, mysql_errno(m_mysql.get()), mysql_error(m_mysql.get()));
    return rt;
}
...
{
    MySQLTransaction::ptr rt = base::protected_make_shared<MySQLTransaction>(mysql, auto_commit);
    if (rt->begin()) {
        return rt;
    }
    return nullptr;
...
{
    if (m_isFinished) {
        _LOG_ERROR(g_logger) << "transaction is finished, format=" << format;
        return -1;
    }
...
{
    if (m_isFinished) {
        _LOG_ERROR(g_logger) << "transaction is finished, sql=" << sql;
        return -1;
    }
    ...
    if (it != m_conns.end()) {
        if (!it->second.empty()) {
            ...
            if (rt->ping()) {
                ...
                _LOG_WARN(g_logger) << "reconnect " << name << " fail";
                return nullptr;
            }
        }
    }
...
int MySQLManager::execute(const std::string &name, const char *format, va_list ap)
{
    auto conn = get(name);
    if (!conn) {
        _LOG_ERROR(g_logger) << "MySQLManager::execute, get(" << name
                             << ") fail, format=" << format;
        return -1;
    }
...
int MySQLManager::execute(const std::string &name, const std::string &sql)
{
    auto conn = get(name);
    if (!conn) {
        _LOG_ERROR(g_logger) << "MySQLManager::execute, get(" << name << ") fail, sql=" << sql;
        return -1;
    }
...
ISQLData::ptr MySQLManager::query(const std::string &name, const char *format, va_list ap)
{
    auto conn = get(name);
    if (!conn) {
        _LOG_ERROR(g_logger) << "MySQLManager::query, get(" << name << ") fail, format=" << format;
        return nullptr;
    }
...
ISQLData::ptr MySQLManager::query(const std::string &name, const std::string &sql)
{
    auto conn = get(name);
    if (!conn) {
        _LOG_ERROR(g_logger) << "MySQLManager::query, get(" << name << ") fail, sql=" << sql;
        return nullptr;
    }
...
MySQLTransaction::ptr MySQLManager::openTransaction(const std::string &name, bool auto_commit)
{
    auto conn = get(name);
    if (!conn) {
        _LOG_ERROR(g_logger) << "MySQLManager::openTransaction, get(" << name << ") fail";
        return nullptr;
    }
...
ISQLData::ptr MySQLUtil::Query(const std::string &name, const char *format, va_list ap)
{
    ...
    if (!m) {
        _LOG_ERROR(g_logger) << "mysql name=" << name << " not exists";
        return nullptr;
    }
