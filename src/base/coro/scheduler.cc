...
Scheduler::Scheduler(size_t threads, bool use_caller, const std::string &name, bool hook_enable) : m_name(name), m_hook_enable(hook_enable)
{
    _ASSERT(threads > 0);
...
void Scheduler::start()
{
    ...
    //     //m_rootFiber->swapIn();
    //     m_rootFiber->call();
    //     _LOG_INFO(g_logger) << "call out " << m_rootFiber->getState();
    // }
}
...
void Scheduler::stop()
{
    ...
    if (m_rootFiber && m_threadCount == 0
        ...
        _LOG_INFO(g_logger) << this << " stopped";
        m_stopping = true;
    ...
    if (m_rootFiber) {
        ...
        //         m_rootFiber.reset(new Fiber(std::bind(&Scheduler::run, this), 0, true));
        //         _LOG_INFO(g_logger) << " root fiber is term, reset";
...
void Scheduler::run()
{
    _LOG_DEBUG(g_logger) << m_name << " run";
    set_hook_enable(m_hook_enable);
    setThis();
    if (base::GetThreadId() != m_rootThread) {
    ...
    while (true) {
        ...
        if (ft.fiber
            ...
            if (idle_fiber->getState() == Fiber::TERM) {
                _LOG_INFO(g_logger) << "idle fiber term";
                break;
            }
