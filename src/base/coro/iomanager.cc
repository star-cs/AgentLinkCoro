...
{
    // _LOG_INFO(g_logger) << "fd=" << fd
    //     << " triggerEvent event=" << event
    //     << " events=" << events;
    // _ASSERT(events & event);
    if (_UNLIKELY(!(events & event))) {
        _LOG_ERROR(g_logger) << "fd=" << fd << " triggerEvent event=" << event
                             << " events=" << events << "\nbacktrace:\n"
...
int IOManager::addEvent(int fd, Event event, std::function<void()> cb)
{
    ...
    if (_UNLIKELY(fd_ctx->events & event)) {
        _LOG_ERROR(g_logger) << "addEvent assert fd=" << fd << " event=" << (EPOLL_EVENTS)event
    ...
    if (rt) {
        _LOG_ERROR(g_logger) << "epoll_ctl(" << m_epfd << ", " << (EpollCtlOp)op << ", " << fd
    ...
    if (cb) {
        ...
        _ASSERT2(event_ctx.fiber->getState() == Fiber::EXEC,
...
void IOManager::idle()
{
    ...
    while (true) {
        ...
        if (_UNLIKELY(stopping(next_timeout))) {
            _LOG_INFO(g_logger) << "name=" << getName() << " idle stopping exit";
            break;
        }
        ...
        listExpiredCb(cbs);
        if (!cbs.empty()) {
            // _LOG_DEBUG(g_logger) << "on timer cbs.size=" << cbs.size();
            schedule(cbs.begin(), cbs.end());
            cbs.clear();
        }

        // if(_UNLIKELY(rt == MAX_EVNETS)) {
        //     _LOG_INFO(g_logger) << "epoll wait events=" << rt;
        // }
        ...
        for (int i = 0; i < rt; ++i) {
            ...
            // _LOG_INFO(g_logger) << " fd=" << fd_ctx->fd << " events=" << fd_ctx->events
