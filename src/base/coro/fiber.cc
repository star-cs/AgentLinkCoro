...
static Logger::ptr g_logger = _LOG_NAME("system");
...
Fiber::Fiber()
{
    ...
    _LOG_DEBUG(g_logger) << "Fiber::Fiber main";
}
    ...
    : m_id(++s_fiber_id), m_cb(cb)
{
    ...
    _LOG_DEBUG(g_logger) << "Fiber::Fiber id=" << m_id;
}

Fiber::~Fiber()
{
    --s_fiber_count;
    if (m_stack != nullptr && m_stacksize != 0) {
        _ASSERT(m_state == TERM || m_state == EXCEPT || m_state == INIT);
    ...
    _LOG_DEBUG(g_logger) << "Fiber::~Fiber id=" << m_id << " total=" << s_fiber_count;
}
...
void *Fiber::MainFunc(void *, void *)
{
    ...
    try {
        ...
        cur->m_state = EXCEPT;
        _LOG_ERROR(g_logger) << "Fiber Except: " << ex.what() << " fiber_id=" << cur->getId()
                             << std::endl
                             << base::BacktraceToString();
    } catch (...) {
        cur->m_state = EXCEPT;
        _LOG_ERROR(g_logger) << "Fiber Except"
    ...
    _ASSERT2(false, "never reach fiber_id=" + std::to_string(raw_ptr->getId()));
}
