...
#define aco_amd64_inline_short_aligned_memcpy(dst, src, sz)                                        \
    do {                                                                                           \
        ...
        *((uint64_t *)((uintptr_t)(dst) + (sz) - 8)) =                                             \
            *((uint64_t *)((uintptr_t)(src) + (sz) - 8));                                          \
    } while (0)
        ...
        if (resume_co->share_stack->owner != NULL) {
            ...
            assert(((uintptr_t)(owner_co->share_stack->align_retptr)
                   ...
                   && ((uintptr_t)(owner_co->share_stack->align_highptr)
                       ...
                       <= (uintptr_t)(owner_co->reg[ACO_REG_IDX_SP])));
            owner_co->save_stack.valid_sz = (uintptr_t)(owner_co->share_stack->align_retptr)
                                            - (uintptr_t)(owner_co->reg[ACO_REG_IDX_SP]);
...
#    ifdef __x86_64__
                aco_amd64_optimized_memcpy_drop_in(owner_co->save_stack.ptr,
                                                   owner_co->reg[ACO_REG_IDX_SP],
                                                   owner_co->save_stack.valid_sz);
#    else
                memcpy(owner_co->save_stack.ptr, owner_co->reg[ACO_REG_IDX_SP],
                       owner_co->save_stack.valid_sz);
#    endif
...
#    else
            memcpy((void *)((uintptr_t)(resume_co->share_stack->align_retptr)
                            - resume_co->save_stack.valid_sz),
                   resume_co->save_stack.ptr, resume_co->save_stack.valid_sz);
#    endif
