#include "base/net/client/consul_client.h"
...
void run()
{
    ...
    base::ConsulRegisterInfo::ptr info = std::make_shared<base::ConsulRegisterInfo>();
    info->id = base::GetConsulUniqID(8080);
    info->name = "test-xxx";
    ...
    info->meta["m1"] = "m2";
    info->port = 8080;
    info->check = std::make_shared<base::ConsulCheck>();
    ...
    std::cout << base::pack::EncodeToJsonString(info, 0) << std::endl;
    std::cout << "register: " << client->serviceRegister(info) << std::endl;

    std::map<std::string, std::list<base::ConsulServiceInfo::ptr> > m;
    if (client->serviceQuery({"test-xxx", "recommend-guard"}, m)) {
        std::cout << base::pack::EncodeToJsonString(m, 0) << std::endl;
    } else {
