...
void run()
{
    do {
        ...
        base::MySQL::ptr mysql(new base::MySQL(params));
        if (!mysql->connect()) {
        ...
        base::MySQLStmt::ptr stmt =
            base::MySQLStmt::Create(mysql, "update user set update_time = ? where id = 1");
        stmt->bindString(1, "2018-01-01 10:10:10");
        int rt = stmt->execute();
        std::cout << "rt=" << rt << std::endl;

        // MYSQL_TIME mt;
        // base::time_t_to_mysql_time(time(0), mt);

        // int a = 0;
        ////auto stmt = mysql->prepare("select * from sylar where status >= ?");
        ////stmt->bind(0, a);
        ////auto res = std::dynamic_pointer_cast<base::MySQLStmtRes>(stmt->query());

        // auto res = std::dynamic_pointer_cast<base::MySQLStmtRes>
        //     //(mysql->queryStmt("select * from sylar"));
        //     (mysql->queryStmt("select *, 'hello' as xx from user where status >= ? and status <=
        //     ?"
        //                       , a, a));
        //     //(mysql->queryStmt("select id,name, keyword, creator as aa, last_update_time from
        //     sylar "
        //     //                  " where last_update_time > ?", (time_t)0));
        ////auto res = std::dynamic_pointer_cast<base::MySQLRes>
